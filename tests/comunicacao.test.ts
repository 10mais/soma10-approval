...
// Quarta-feira, 09/09/2026, meio-dia local.
...
describe('comunicação — a semana', () => {
  ...
  it('doDia separa por dia e diasUteisSemComunicar ignora o fim de semana', () => {
    const ontem = com({ id: 'a', em: '2026-09-08T09:00:00.000Z' })
    ...
    expect(diasUteisSemComunicar([com({ em: '2026-09-04T15:00:00.000Z' })], AGORA)).toBe(3)
    expect(diasUteisSemComunicar([], AGORA)).toBeGreaterThan(3)
  })
})

describe('comunicação — candidatos, sem repetir e podendo evoluir', () => {
  const post = { id: 'p1', headline: 'Reel da reciclagem', etapa: 'aprovacao_criativo', status: 'aguardando_aprovacao', aguardandoDesde: '2026-09-08T12:00:00.000Z' }
  ...
  it('quando o assunto EVOLUI, ele volta — e sabe quantas vezes já foi falado', () => {
    const feito = com({ assunto: 'post:p1', estado: 'aguardando:aprovacao_criativo' })
    const publicado = { ...post, status: 'publicado', publicadoEm: '2026-09-09T11:30:00.000Z' }
  ...
  it('material parado há dias vira questionamento com consequência e prazo, uma vez por semana', () => {
    const parado = { ...post, aguardandoDesde: '2026-09-01T12:00:00.000Z' }
  ...
  it('tarefa concluída, etapa e marco do Playbook viram candidatos; reunião só hoje ou amanhã', () => {
    const c = candidatosDoDia({
      tarefas: [{ id: 't1', titulo: 'Auditoria dos perfis', status: 'concluido', concluidoEm: '2026-09-09T10:00:00.000Z' }],
      marcos: [
        { id: 'm1', titulo: 'Lançamento', status: 'em_andamento', subetapas: [{ id: 's1', titulo: 'Setup', status: 'concluido', dataFim: '2026-09-08' }] },
        { id: 'm2', titulo: 'Alinhamento mensal', categoria: 'reuniao', dataInicio: '2026-09-10T14:00:00.000Z' },
        { id: 'm3', titulo: 'Reunião antiga', categoria: 'reuniao', dataInicio: '2026-08-10T14:00:00.000Z' },
      ],
      agora: AGORA,
    })
  ...
  it('os dias futuros da semana já mostram o que está previsto', () => {
    ...
    const p = previstoNaSemana({
      posts: [{ id: 'p9', headline: 'Carrossel', dataAgendada: '2026-09-11T11:30:00.000Z', status: 'agendado' }],
      tarefas: [{ id: 't9', titulo: 'Enviar relatório', status: 'a_fazer', prazo: '2026-09-10T23:59:00.000Z' }],
