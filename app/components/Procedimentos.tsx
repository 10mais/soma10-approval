  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <h2 style={{ margin: 0, fontSize: 20, color: 'var(--v2-ink)' }}>Procedimentos e Métodos</h2>
        <span style={{ flex: 1 }} />
        {podeEditar && <button onClick={() => setForm(vazio())} style={{ padding: '9px 16px', background: 'var(--v2-ink)', color: 'var(--v2-surface)', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>+ Procedimento</button>}
      </div>
      <p style={{ margin: '0 0 16px', fontSize: 13, color: 'var(--v2-ink3)' }}>Tudo o que a clínica atende. A Agenda e o registro pós-atendimento usam esta lista.</p>

      {carregando ? <p style={{ color: 'var(--v2-ink3)', fontSize: 13 }}>Carregando...</p>
        : lista.length === 0 ? <p style={{ color: 'var(--v2-ink3)', fontSize: 13 }}>Nenhum procedimento cadastrado ainda. {podeEditar ? 'Comece pelo botão "+ Procedimento".' : ''}</p>
        : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            {Object.entries(grupos).map(([cat, itens]) => (
              <div key={cat}>
                <h3 style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 800, color: 'var(--v2-ink3)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{cat}</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {itens.map(p => (
                    <div key={p.id} onClick={() => podeEditar && setForm({ id: p.id, nome: p.nome, categoria: p.categoria || '', valorPadrao: p.valorPadrao ? String(p.valorPadrao) : '', duracaoMin: p.duracaoMin ? String(p.duracaoMin) : '', descricao: p.descricao || '' })}
                      style={{ background: 'var(--v2-surface)', border: '1px solid var(--v2-rule)', borderRadius: 12, padding: '12px 14px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', cursor: podeEditar ? 'pointer' : 'default' }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                        <span style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--v2-ink)' }}>{p.nome}</span>
                        <span style={{ flex: 1 }} />
                        {p.duracaoMin ? <span style={{ fontSize: 12, color: 'var(--v2-ink3)' }}>{p.duracaoMin} min</span> : null}
                        {p.valorPadrao ? <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--v2-ok)' }}>{fmtBRL(p.valorPadrao)}</span> : null}
                      </div>
                      {p.descricao && <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--v2-ink3)' }}>{p.descricao}</p>}
                    </div>
                  ))}
                </div>
              </div>
      ...
      {form && (
        <div onClick={fecharFora(() => setForm(null))} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div onClick={ev => ev.stopPropagation()} style={{ background: 'var(--v2-surface)', borderRadius: 16, maxWidth: 460, width: '100%', maxHeight: '92vh', overflowY: 'auto', padding: 22 }}>
            <h3 style={{ margin: '0 0 14px', fontSize: 16.5, color: 'var(--v2-ink)' }}>{form.id ? 'Editar procedimento' : 'Novo procedimento'}</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <textarea lang="pt-BR" value={form.descricao} onChange={e => setForm(f => f && ({ ...f, descricao: e.target.value }))} placeholder="Descrição / observações (opcional)" rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 16, alignItems: 'center' }}>
              {form.id && <button onClick={() => { const p = lista.find(x => x.id === form.id); if (p) excluir(p) }} style={{ padding: '9px 14px', background: 'var(--v2-surface)', border: '1px solid var(--v2-hot-bg)', borderRadius: 9, color: 'var(--v2-hot)', fontSize: 12.5, fontWeight: 700, cursor: 'pointer', marginRight: 'auto' }}>Excluir</button>}
