...
const secaoStyle: React.CSSProperties = { fontSize: 13, fontWeight: 800, color: 'var(--v2-ink)' }
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
            {lista.map(p => (
              <div key={p.id} onClick={() => podeEditar && abrirEditar(p)} style={{ background: 'var(--v2-surface)', border: '1px solid var(--v2-rule)', borderRadius: 12, padding: 14, boxShadow: '0 1px 4px rgba(0,0,0,0.05)', cursor: podeEditar ? 'pointer' : 'default', opacity: p.ativo === false ? 0.6 : 1 }}>
                ...
                <div style={{ fontSize: 12.5, color: 'var(--v2-ink2)', marginTop: 4 }}>
                  {p.destino || 'sem destino'}
                  {p.dias ? ` · ${p.dias} dia(s)` : ''}{p.noites ? ` / ${p.noites} noite(s)` : ''}
                </div>
                <div style={{ fontSize: 11.5, color: 'var(--v2-ink3)', marginTop: 6 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ borderTop: '1px solid var(--v2-rule)', paddingTop: 12 }}>
                ...
                {form.roteiroPadrao.filter(p => p.titulo.trim()).length > 0 && (
                  <div style={{ marginTop: 8, padding: 10, background: 'var(--v2-surface1)', borderRadius: 10, border: '1px solid var(--v2-rule)' }}>
                    {porDia(form.roteiroPadrao.filter(p => p.titulo.trim())).map(({ dia, paradas }) => (
                      <div key={dia} style={{ marginBottom: 4 }}>
                        <span style={{ fontSize: 11, fontWeight: 800, color: 'var(--v2-ink)' }}>Dia {dia}: </span>
                        <span style={{ fontSize: 11.5, color: 'var(--v2-ink2)' }}>{paradas.map(p => `${p.hora ? p.hora + ' ' : ''}${p.titulo}`).join(' · ')}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div style={{ borderTop: '1px solid var(--v2-rule)', paddingTop: 12 }}>
                ...
                {calc && calc.resultado.custo > 0 && (
                  <div style={{ marginTop: 10, padding: 12, background: 'var(--v2-surface1)', borderRadius: 10, border: '1px solid var(--v2-rule)' }}>
                    <div style={{ fontSize: 12.5, color: 'var(--v2-ink)', marginBottom: 6 }}>
                    ...
                    {calc.pax > 0 && Number(form.valorBase) > 0 && (
                      <div style={{ fontSize: 12, color: 'var(--v2-ink2)' }}>
                        ...
                        <strong style={{ color: calc.resultado.resultado >= 0 ? 'var(--v2-ok)' : 'var(--v2-hot)' }}>
