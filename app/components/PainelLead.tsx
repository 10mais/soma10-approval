...
export default function PainelLead({
  ...
  async function chamarAssistente() {
    ...
    else toast(r?.error || tr('lead.falha-assistente'), 'erro')
  }
  ...
  const inserir = (texto: string) => { onInserir(aplicarPlaceholders(texto, contato?.nome || nome)); toast(tr('lead.no-compositor'), 'info') }
  ...
  async function concluirPasso(id: string) {
    ...
    if (r?.ok) onContatoMudou?.(); else toast(tr('lead.falha-concluir'), 'erro')
  }
  ...
  return (
    <div style={{ width: movel ? '100%' : 330, borderLeft: movel ? 'none' : '1px solid var(--v2-surface2)', display: 'flex', flexDirection: 'column', flexShrink: 0, background: 'var(--v2-surface)', minWidth: 0 }}>
      ...
      <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--v2-rule)', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--v2-ink)' }}>{tr('lead.raio-x-lead')}</span>
        <span style={{ flex: 1 }} />
        <button onClick={onFechar} title={tr('lead.fechar-painel')} style={{ background: 'none', border: 'none', fontSize: 18, color: 'var(--v2-ink3)', cursor: 'pointer', lineHeight: 1, padding: 0 }}>×</button>
      </div>
      <div style={{ display: 'flex', borderBottom: '1px solid var(--v2-rule)', flexShrink: 0 }}>
        {([['geral', 'Geral'], ['followup', 'Follow-up'], ['paciente', tr('lead.paciente')], ['financeiro', tr('lead.financeiro')]] as [Aba, string][]).map(([k, label]) => (
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {aba === 'geral' && (<>
          ...
          <div style={{ ...secao, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div><span style={rotulo}>{tr('lead.entrou-contato')}</span><span style={valor}>{dataHora(resumo.primeiroEm)}</span></div>
            <div><span style={rotulo}>{tr('lead.ultima-mensagem')}</span><span style={valor}>{dataHora(resumo.ultimaEm)}</span></div>
            <div><span style={rotulo}>{tr('lead.quem-falou-por-ultimo')}</span><span style={valor}>{resumo.ultimaDe === 'cliente' ? (contato?.nome || nome || tr('lead.a-pessoa')) : resumo.ultimaDe === 'agente' ? 'Nós' : '—'}</span></div>
            <div><span style={rotulo}>{tr('crm.mensagens')}</span><span style={valor}>{resumo.total} <span style={{ fontWeight: 400, color: 'var(--v2-ink3)' }}>({resumo.doCliente} dela)</span></span></div>
          </div>

          <div style={secao}>
            <span style={rotulo}>{tr('lead.interesses-que-ela-pediu')}</span>
            {interesses.length === 0 ? (
              <p style={{ margin: '6px 0 0', fontSize: 11.5, color: 'var(--v2-ink3)' }}>{tr('lead.nenhum-procedimento-catalogo-f')}</p>
            ) : (
          ...
          <div style={secao}>
            <span style={rotulo}>{tr('lead.assistente')}</span>
            <p style={{ margin: '5px 0 8px', fontSize: 11.5, color: 'var(--v2-ink3)', lineHeight: 1.5 }}>{tr('lead.ela-le-conversa-responde-pelo')}</p>
            <input value={foco} onChange={e => setFoco(e.target.value)} placeholder={tr('lead.opcional-que-voce-precisa-ex-o')}
            ...
            <button onClick={chamarAssistente} disabled={pensando}
              ...
              {pensando ? tr('lead.lendo') : orientacao ? tr('lead.pedir-de-novo') : tr('lead.pedir-orientacao')}
            </button>
            ...
            {orientacao && (
              <div style={{ marginTop: 10, background: 'var(--v2-surface1)', borderRadius: 12, padding: 11 }}>
                ...
                {orientacao.proximaAcao && (<>
                  <span style={{ ...rotulo, marginTop: 10 }}>{tr('lead.proxima-acao')}</span>
                ...
                {orientacao.mensagem && (<>
                  <span style={{ ...rotulo, marginTop: 10 }}>{tr('lead.mensagem-sugerida')}</span>
                  ...
                  <p style={{ margin: '6px 0 0', fontSize: 10.5, color: 'var(--v2-ink3)', lineHeight: 1.4 }}>{tr('lead.ela-vai-compositor-leia-ajuste')}</p>
                </>)}
              </div>
            )}
          </div>
        ...
        {aba === 'followup' && (<>
          <div style={secao}>
            <span style={rotulo}>{tr('lead.follow-up-agendado')}</span>
            {!contato?.id ? (
              <p style={{ margin: '6px 0 0', fontSize: 11.5, color: 'var(--v2-ink3)' }}>{tr('lead.vincule-esta-conversa-contato')}</p>
            ) : (<>
              {passosAbertos.length === 0 && <p style={{ margin: '6px 0 8px', fontSize: 11.5, color: 'var(--v2-ink3)' }}>{tr('lead.nenhum-retorno-marcado')}</p>}
              {passosAbertos.map(p => (
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderTop: '1px solid var(--v2-surface1)' }}>
                  <button onClick={() => concluirPasso(p.id)} title={tr('lead.marcar-como-feito')}
              ...
              {podeEditar && (
                <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                  <input value={novoPasso.titulo} onChange={e => setNovoPasso({ ...novoPasso, titulo: e.target.value })} placeholder={tr('lead.que-fazer-ex-chamar-novo')}
          ...
          {bib.cadencias.map(c => (
            <div key={c.id} style={secao}>
              ...
              {c.mensagens.map(m => {
                ...
                return (
                  <button key={m.id} onClick={() => inserir(m.texto)} title={tr('lead.colocar-compositor')} style={botaoTexto}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      {fase && <span style={{ fontSize: 9.5, fontWeight: 800, color: fase.cor }}>{tr(fase.rotulo).toUpperCase()}</span>}
          ...
          {[...bib.reaquecimento.leads, ...bib.reaquecimento.clientes].length > 0 && (
            <div style={secao}>
              <span style={rotulo}>{tr('lead.reaquecimento')}</span>
          ...
          {bib.objecoes.length > 0 && (
            <div style={secao}>
              <span style={rotulo}>{tr('lead.objecoes')}</span>
              {bib.objecoes.map(cat => (
                <details key={cat.id} style={{ marginTop: 7 }}>
                  ...
                  <div style={{ marginTop: 6 }}>
                    {cat.respostas.map(r => (
                      <button key={r.id} onClick={() => inserir(r.texto)} title={tr('lead.colocar-compositor')} style={botaoTexto}>
          ...
          {bib.cadencias.length === 0 && bib.objecoes.length === 0 && (
            <div style={secao}>
              <p style={{ margin: 0, fontSize: 11.5, color: 'var(--v2-ink3)', lineHeight: 1.5 }}>{tr('lead.biblioteca-vendas-ainda-esta-v')}</p>
            </div>
          )}
        </>)}
        ...
        {aba === 'paciente' && (<>
          {!contato ? (
            <div style={secao}>
              <p style={{ margin: 0, fontSize: 11.5, color: 'var(--v2-ink3)', lineHeight: 1.5 }}>{tr('lead.esta-conversa-ainda-nao-esta-v')}</p>
            </div>
          ) : (<>
            <div style={{ ...secao, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div><span style={rotulo}>{tr('crm.nome')}</span><span style={valor}>{contato.nome}</span></div>
              <div><span style={rotulo}>{tr('crm.telefone')}</span><span style={valor}>{contato.telefone || '—'}</span></div>
              <div><span style={rotulo}>{tr('comum.tipo')}</span><span style={valor}>{contato.tipo || '—'}</span></div>
              <div><span style={rotulo}>{tr('crm.nascimento')}</span><span style={valor}>{contato.nascimento ? dataBr(contato.nascimento) : '—'}</span></div>
              <div><span style={rotulo}>{tr('lead.situacao')}</span><span style={valor}>{contato.ativo === false ? tr('lead.inativa') : 'Ativa'}</span></div>
              <div><span style={rotulo}>{tr('crm.ultimo-procedimento')}</span><span style={valor}>{contato.ultimoProcedimento || '—'}</span></div>
            </div>
            {contato.observacoes && (
              <div style={secao}><span style={rotulo}>{tr('crm.observacoes')}</span><p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--v2-ink2)', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{contato.observacoes}</p></div>
            )}
            <div style={secao}>
              <span style={rotulo}>Atendimentos ({atendimentos.length})</span>
              {atendimentos.length === 0 && <p style={{ margin: '6px 0 0', fontSize: 11.5, color: 'var(--v2-ink3)' }}>{tr('lead.nenhum-atendimento-registrado')}</p>}
              {atendimentos.slice(0, 12).map(a => (
                <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderTop: '1px solid var(--v2-surface1)' }}>
                  ...
                  <span style={{ flex: 1, minWidth: 0, fontSize: 12, color: 'var(--v2-ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.servico || tr('lead.atendimento')}{a.profissionalNome ? ` · ${a.profissionalNome}` : ''}</span>
            ...
            <div style={{ ...secao, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {onAbrirFicha && <button onClick={onAbrirFicha} style={{ flex: 1, padding: '9px 10px', background: 'var(--v2-surface)', border: '1px solid var(--v2-rule)', borderRadius: 9, fontWeight: 700, fontSize: 12, cursor: 'pointer', color: 'var(--v2-ink)' }}>{tr('lead.abrir-ficha-completa')}</button>}
              {onAgendar && <button onClick={onAgendar} style={{ flex: 1, padding: '9px 10px', background: 'var(--marca, var(--v2-amber-on))', color: 'var(--marca-texto, var(--v2-ink))', border: 'none', borderRadius: 9, fontWeight: 800, fontSize: 12, cursor: 'pointer' }}>{tr('lead.agendar')}</button>}
            </div>
          </>)}
        </>)}
        ...
        {aba === 'financeiro' && (<>
          {!contato ? (
            <div style={secao}><p style={{ margin: 0, fontSize: 11.5, color: 'var(--v2-ink3)', lineHeight: 1.5 }}>{tr('lead.vincule-conversa-contato-ver-o')}</p></div>
          ) : (<>
            <div style={{ ...secao, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div><span style={rotulo}>{tr('lead.ja-fechou')}</span><span style={{ ...valor, color: 'var(--v2-ok)', fontSize: 15, fontWeight: 800 }}>{brl(totalGanho)}</span></div>
              <div><span style={rotulo}>{tr('crm.em-aberto')}</span><span style={{ ...valor, color: 'var(--v2-ink)', fontSize: 15, fontWeight: 800 }}>{brl(emAberto)}</span></div>
              <div><span style={rotulo}>{tr('lead.oportunidades')}</span><span style={valor}>{meus.length}</span></div>
              <div><span style={rotulo}>{tr('lead.perdidas')}</span><span style={valor}>{perdidos.length}</span></div>
            </div>
            <div style={secao}>
              <span style={rotulo}>{tr('lead.oportunidades-desta-pessoa')}</span>
              {meus.length === 0 && <p style={{ margin: '6px 0 0', fontSize: 11.5, color: 'var(--v2-ink3)' }}>{tr('lead.nenhuma-oportunidade-ainda')}</p>}
              {meus.map(n => (
                <div key={n.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0', borderTop: '1px solid var(--v2-surface1)' }}>
                  ...
                  <span style={{ flex: 1, minWidth: 0, fontSize: 12, color: 'var(--v2-ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{n.titulo || tr('lead.oportunidade')}</span>
            ...
            {onAbrirOportunidade && (
              <div style={secao}>
                <button onClick={onAbrirOportunidade} style={{ width: '100%', padding: '9px 10px', background: 'var(--marca, var(--v2-amber-on))', color: 'var(--marca-texto, var(--v2-ink))', border: 'none', borderRadius: 9, fontWeight: 800, fontSize: 12, cursor: 'pointer' }}>{tr('crm.nova-oportunidade')}</button>
              </div>
            )}
          </>)}
        </>)}
