...
// Editor de ROTEIRO (itinerário) de uma excursão — modal dedicado. Monta a linha
// do tempo dia-a-dia entre dataIda e dataVolta; cada parada entra num dia + hora.
// Salva em Excursao.paradas via PUT /api/excursoes.
...
export default function RoteiroExcursao({ excursao, podeEditar = true, onClose, onSaved }: {
  excursao: { id: string; titulo: string; dataIda: string; dataVolta?: string; paradas?: Parada[] }
  podeEditar?: boolean
  onClose: () => void
  onSaved: () => void
}) {
  const [paradas, setParadas] = useState<Parada[]>(() => (excursao.paradas || []).map(p => ({ ...p })))
  const [salvando, setSalvando] = useState(false)
  const inicial = useRef(JSON.stringify(excursao.paradas || []))
  const idRef = useRef(0)
  const novoId = () => `p${Date.now()}_${idRef.current++}`

  const dias = useMemo(() => diasDoIntervalo(excursao.dataIda, excursao.dataVolta), [excursao.dataIda, excursao.dataVolta])
  ...
  async function salvar() {
    ...
    const r = await fetch('/api/excursoes', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: excursao.id, paradas: limpa }) }).then(x => x.json()).catch(() => null)
    setSalvando(false)
    ...
    <div onClick={fechar} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, maxWidth: 680, width: '100%', maxHeight: '92vh', overflowY: 'auto', padding: 22 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 13, color: '#888' }}>{excursao.titulo}</span>
        </div>
        <p style={{ margin: '0 0 16px', fontSize: 12.5, color: '#999' }}>Monte a linha do tempo do período. Cada parada entra no dia e horário.</p>

        {dias.length === 0 ? (
          <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 12, padding: 16, fontSize: 13, color: '#92400e' }}>Defina a <strong>data de ida</strong> (e volta) da excursão para montar o roteiro.</div>
        ) : (
