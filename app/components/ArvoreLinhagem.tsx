...
export default function ArvoreLinhagem({ pessoas, compacta = false }: { pessoas: PessoaLinhagem[]; compacta?: boolean }) {
  if (!pessoas.length) {
    return <p style={{ margin: 0, fontSize: 12.5, color: 'var(--v2-ink3)' }}>Nenhuma pessoa na linhagem ainda.</p>
  }
  ...
  return (
    <div>
      {lacunas.length > 0 && (
        <div style={{ marginBottom: 10, padding: '7px 10px', background: 'var(--v2-amber-bg)', border: '1px solid var(--v2-amber-bg)', borderRadius: 8, fontSize: 11.5, color: 'var(--v2-amber)' }}>
        ...
        {ordenada.map((p, idx) => {
          ...
          return (
            <div key={p.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: 360 }}>
              <div style={{ width: '100%', background: ehAsc ? 'var(--v2-info-bg)' : 'var(--v2-surface)', border: '1px solid ' + (ehAsc ? '#c7d2fe' : 'var(--v2-surface2)'), borderRadius: 10, padding: compacta ? '7px 10px' : '9px 12px', textAlign: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, flexWrap: 'wrap' }}>
                  <strong style={{ fontSize: 13, color: 'var(--v2-ink)' }}>{p.nome}</strong>
                ...
                <div style={{ fontSize: 11, color: 'var(--v2-ink3)', marginTop: 2 }}>
                  {p.papel || `Geração ${p.geracao}`}{p.nascimentoLocal ? `  ·  ${p.nascimentoLocal}` : ''}
                </div>
                {!compacta && fmtVida(p) && <div style={{ fontSize: 11, color: 'var(--v2-ink3)', marginTop: 2 }}>{fmtVida(p)}</div>}
              </div>
              {idx < ordenada.length - 1 && <div style={{ width: 2, height: 16, background: 'var(--v2-rule)' }} />}
            </div>
          )
        })}
      </div>
