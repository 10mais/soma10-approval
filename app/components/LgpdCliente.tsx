...
export default function LgpdCliente({ clienteId, clienteNome, onApagado }: { clienteId: string; clienteNome: string; onApagado?: () => void }) {
  ...
  const btn: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 9, fontWeight: 700, fontSize: 12.5, color: '#374151', cursor: 'pointer', textDecoration: 'none' }
  ...
  return (
    <div>
      <span style={{ display: 'block', fontSize: 11, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '16px 0 8px', paddingTop: 16, borderTop: '1px dashed #e5e7eb' }}>Privacidade (LGPD)</span>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setAbrindo(v => !v)} style={{ ...btn, color: '#b91c1c', borderColor: '#fecaca' }}>
      ...
      {abrindo && (
        <div style={{ marginTop: 10, padding: 12, border: '1.5px solid #fecaca', borderRadius: 10, background: '#fef7f7' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button onClick={apagar} disabled={!nomeBate || apagando} style={{ padding: '8px 16px', background: nomeBate ? '#b91c1c' : '#eee', color: nomeBate ? '#fff' : '#aaa', border: 'none', borderRadius: 8, fontWeight: 800, fontSize: 12.5, cursor: nomeBate && !apagando ? 'pointer' : 'not-allowed' }}>{apagando ? 'Apagando…' : 'Apagar definitivamente'}</button>
          </div>
        </div>
      )}
    </div>
