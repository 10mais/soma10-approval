...
const campo: React.CSSProperties = { padding: '8px 10px', borderRadius: 9, border: '1.5px solid var(--v2-rule)', fontSize: 12.5, fontFamily: 'inherit', boxSizing: 'border-box' }
const rotulo: React.CSSProperties = { display: 'block', fontSize: 11.5, fontWeight: 700, color: 'var(--v2-ink3)', marginBottom: 6 }
...
export default function LancarGanhoModal({ ganho, catalogo, onClose, onLancado }: {
  ...
  return (
    <div onClick={fecharFora(onClose)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, padding: 20 }}>
      <div onClick={e => e.stopPropagation()} className="soma10-no-invert" style={{ background: 'var(--v2-surface)', borderRadius: 16, maxWidth: 600, width: '100%', maxHeight: '90vh', overflowY: 'auto', padding: 22 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <h3 style={{ margin: 0, fontSize: 16, color: 'var(--v2-ink)' }}>Lançar no caixa</h3>
            <p style={{ margin: '3px 0 0', fontSize: 12.5, color: 'var(--v2-ink3)' }}>{ganho.descricao}</p>
          </div>
          <span style={{ fontSize: 18, fontWeight: 800, color: 'var(--v2-ok)', flexShrink: 0 }}>{brl(ganho.valor)}</span>
          <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: 'var(--v2-ink3)', lineHeight: 1, padding: 0 }}>×</button>
        </div>

        {/* O QUE FOI VENDIDO — vem do CRM e pode ser corrigido aqui */}
        <div style={{ marginTop: 18 }}>
          <label style={rotulo}>Procedimento / método</label>
          {procedimentos.length === 0 && <p style={{ margin: '0 0 6px', fontSize: 11.5, color: 'var(--v2-ink3)' }}>Nada marcado na oportunidade — escolha abaixo (vale também para o CRM).</p>}
          {procedimentos.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 7 }}>
              {procedimentos.map(p => (
                <span key={p} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11.5, fontWeight: 700, color: '#3730a3', background: 'var(--v2-info-bg)', padding: '4px 8px', borderRadius: 999 }}>
                  {p}
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {naoUsados.length > 0 && (
              <select value="" onChange={e => { if (e.target.value) setProcedimentos(ps => [...ps, e.target.value]) }} style={{ ...campo, flex: 1, minWidth: 180, background: 'var(--v2-surface)' }}>
            ...
            {outro.trim() && (
              <button onClick={() => { setProcedimentos(ps => [...ps, outro.trim()]); setOutro('') }}
                style={{ padding: '8px 12px', background: 'var(--v2-ink)', color: 'var(--v2-surface)', border: 'none', borderRadius: 9, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>+</button>
            )}
          </div>
        </div>
            ...
            {partes.map((p, i) => (
              <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                <select value={p.forma} onChange={e => mudarParte(i, { forma: e.target.value, ...(e.target.value !== 'credito' ? { parcelas: '1' } : {}) })}
                  style={{ ...campo, flex: 1, minWidth: 150, background: 'var(--v2-surface)' }}>
                  <option value="">Forma de pagamento…</option>
                  {FORMAS_PAGAMENTO.map(f => <option key={f.chave} value={f.chave}>{f.label}</option>)}
                </select>
                <div style={{ position: 'relative', width: 130 }}>
                  <span style={{ position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', fontSize: 11.5, fontWeight: 700, color: 'var(--v2-ink3)', pointerEvents: 'none' }}>R$</span>
                ...
                {p.forma === 'credito' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    ...
                    <span style={{ fontSize: 12, color: 'var(--v2-ink3)', fontWeight: 700 }}>x</span>
                  </div>
                )}
                {partes.length > 1 && (
                  <button onClick={() => setPartes(ps => ps.filter((_, idx) => idx !== i))} title="Remover esta forma"
                    style={{ background: 'none', border: 'none', color: 'var(--v2-ink3)', cursor: 'pointer', fontSize: 16, padding: '0 2px' }}>×</button>
                )}
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8, flexWrap: 'wrap' }}>
            <button onClick={adicionarForma} style={{ padding: '7px 12px', background: 'var(--v2-surface1)', border: 'none', borderRadius: 9, fontWeight: 700, fontSize: 12, cursor: 'pointer', color: 'var(--v2-ink)' }}>
              + Outra forma
            </button>
            <span style={{ flex: 1 }} />
            <span style={{ fontSize: 12, color: 'var(--v2-ink3)' }}>Somado: <b style={{ color: Math.abs(soma - ganho.valor) < 0.02 ? 'var(--v2-ok)' : 'var(--v2-hot)' }}>{brl(soma)}</b> de {brl(ganho.valor)}</span>
          </div>
        </div>
        ...
        <div style={{ marginTop: 16, display: 'flex', gap: 10, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <p style={{ flex: 1, minWidth: 220, margin: 0, fontSize: 11, color: 'var(--v2-ink3)', lineHeight: 1.5 }}>
        ...
        <div style={{ marginTop: 16, background: 'var(--v2-surface1)', borderRadius: 12, padding: 12 }}>
          {erro ? (
            <p style={{ margin: 0, fontSize: 12.5, color: 'var(--v2-hot)', fontWeight: 600 }}>{erro}</p>
          ) : (<>
            <p style={{ margin: '0 0 8px', fontSize: 11.5, fontWeight: 800, color: 'var(--v2-ink2)' }}>
            ...
            <div style={{ maxHeight: 170, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 3 }}>
              {previa.map((pc, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--v2-ink2)' }}>
                  <span style={{ width: 74, flexShrink: 0, fontWeight: 700, color: 'var(--v2-ink3)' }}>{dataBr(pc.data)}</span>
                  <span style={{ flex: 1, minWidth: 0 }}>{rotuloFormaPagamento(pc.forma)}{pc.totalParcelas ? ` · ${pc.parcela}/${pc.totalParcelas}` : ''}</span>
                  <span style={{ fontWeight: 800, color: 'var(--v2-ok)' }}>{brl(pc.valor)}</span>
                </div>
              ))}
            </div>
          </>)}
        ...
        {erroServidor && <p style={{ margin: '10px 0 0', fontSize: 12.5, color: 'var(--v2-hot)', fontWeight: 600 }}>{erroServidor}</p>}
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
          ...
          <button onClick={confirmar} disabled={!!erro || salvando}
            style={{ padding: '10px 18px', background: erro || salvando ? 'var(--v2-surface2)' : 'var(--v2-ok)', color: erro || salvando ? 'var(--v2-ink3)' : 'var(--v2-surface)', border: 'none', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: erro || salvando ? 'not-allowed' : 'pointer' }}>
    ...
    <div onClick={fecharFora(onClose)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, padding: 20 }}>
      <div onClick={e => e.stopPropagation()} className="soma10-no-invert" style={{ background: 'var(--v2-surface)', borderRadius: 16, maxWidth: 460, width: '100%', padding: 22 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          <h3 style={{ margin: 0, fontSize: 16, color: 'var(--v2-ink)' }}>Editar lançamento</h3>
        ...
        {naoUsados.length > 0 && (
          <select value="" onChange={e => { if (e.target.value) setProcedimentos(ps => [...ps, e.target.value]) }} style={{ ...campo, width: '100%', background: 'var(--v2-surface)' }}>
            <option value="">Adicionar do catálogo…</option>
            {naoUsados.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        )}
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 16, cursor: 'pointer' }}>
          ...
          <span style={{ fontSize: 12.5, color: 'var(--v2-ink)', fontWeight: 600 }}>Já caiu no caixa</span>
        </label>

        {erro && <p style={{ margin: '10px 0 0', fontSize: 12.5, color: 'var(--v2-hot)', fontWeight: 600 }}>{erro}</p>}
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
          <button onClick={onClose} style={{ padding: '10px 16px', background: 'var(--v2-surface)', border: '1.5px solid var(--v2-rule)', borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: 'pointer', color: 'var(--v2-ink2)' }}>Cancelar</button>
          <button onClick={salvar} disabled={salvando || !descricao.trim()}
            style={{ padding: '10px 18px', background: descricao.trim() ? 'var(--marca, var(--v2-amber-on))' : 'var(--v2-surface2)', color: descricao.trim() ? 'var(--marca-texto, var(--v2-ink))' : 'var(--v2-ink3)', border: 'none', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: descricao.trim() ? 'pointer' : 'not-allowed' }}>
