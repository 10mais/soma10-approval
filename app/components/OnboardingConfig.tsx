...
export default function OnboardingConfig({ onSalvo }: { onSalvo?: () => void } = {}) {
  ...
  const tirarFase = async (i: number) => {
    const f = cfg.fases[i]
    if (f.etapas.length) {
      const ok = await confirmar(`"${f.nome || '—'}" · ${f.etapas.length} ${tr(f.etapas.length === 1 ? 'cfg.etapa' : 'cfg.etapas')}. ${tr('cfg.remover-fase-aviso')}`, { titulo: 'Remover fase', okLabel: 'Remover', perigo: true })
      if (!ok) return
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div>
        <p style={{ margin: 0, fontSize: 15, fontWeight: 500 }}>{tr('cfg.fases-titulo')}</p>
        <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--v2-ink3)', lineHeight: 1.5 }}>
          {tr('cfg.fases-ajuda')}
        </p>
      </div>

      {carregando ? <p style={{ color: 'var(--v2-ink3)', fontSize: 13 }}>{tr('cfg.carregando')}</p> : (
              ...
              <div key={i} style={{ border: '1px solid var(--v2-rule)', borderRadius: 12, background: 'var(--v2-surface)', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px 8px 14px' }}>
                  <button type="button" onClick={() => setAberta(on ? -1 : i)} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 12, background: 'none', border: 0, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', color: 'var(--v2-ink)', padding: '4px 0', minHeight: 32 }}>
                    ...
                    <span style={{ fontSize: 12, color: 'var(--v2-ink3)', fontVariantNumeric: 'tabular-nums' }}>{feitas} {tr(feitas === 1 ? 'cfg.etapa' : 'cfg.etapas')}</span>
                  ...
                  <button type="button" title={tr('cfg.subir-fase')} onClick={() => setFases(fs => mover(fs, i, -1))} disabled={i === 0} style={{ ...mini, opacity: i === 0 ? 0.35 : 1 }}>↑</button>
                  <button type="button" title={tr('cfg.descer-fase')} onClick={() => setFases(fs => mover(fs, i, 1))} disabled={i === cfg.fases.length - 1} style={{ ...mini, opacity: i === cfg.fases.length - 1 ? 0.35 : 1 }}>↓</button>
                  <button type="button" title={tr('cfg.remover-fase')} onClick={() => tirarFase(i)} style={{ ...mini, color: 'var(--v2-hot)' }}>×</button>
                </div>
                {on && (
                  <div style={{ padding: '12px 14px 14px', display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid var(--v2-rule)' }}>
                    <label style={{ fontSize: 10.5, fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--v2-ink3)' }}>{tr('cfg.nome-fase')}</label>
                    <input value={f.nome} onChange={e => mudarFase(i, { nome: e.target.value })} placeholder={tr('cfg.ex-fase')} style={{ ...campo, width: '100%', maxWidth: 420 }} />
                    <label style={{ fontSize: 10.5, fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--v2-ink3)', marginTop: 4 }}>{tr('cfg.etapas-da-fase')}</label>
                    {f.etapas.length === 0 && <p style={{ margin: 0, fontSize: 13, color: 'var(--v2-ink3)' }}>{tr('cfg.sem-etapa')}</p>}
                    {f.etapas.map((e, j) => (
                      <div key={j} style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 2fr) minmax(0, 1.4fr) minmax(0, 1.4fr) auto', gap: 8, alignItems: 'center' }}>
                        <input value={e.nome} onChange={ev => mudarEtapa(i, j, { nome: ev.target.value })} placeholder={tr('cfg.ex-etapa')} style={{ ...campo, minWidth: 0 }} aria-label={tr('cfg.nome-etapa')} />
                        <input value={e.dica || ''} onChange={ev => mudarEtapa(i, j, { dica: ev.target.value })} placeholder={tr('cfg.dica-onde')} style={{ ...campo, minWidth: 0 }} aria-label={tr('cfg.dica')} />
                        <select value={e.auto || ''} onChange={ev => mudarEtapa(i, j, { auto: (ev.target.value || undefined) as Detector | undefined })} style={{ ...campo, minWidth: 0 }} aria-label={tr('cfg.tipo-etapa')}>
                          <option value="">{tr('cfg.manual')}</option>
                          {DETECTORES.map(d => <option key={d.chave} value={d.chave}>{tr('cfg.automatica')}: {d.label}</option>)}
                        </select>
                        <div style={{ display: 'flex', gap: 4 }}>
                          <button type="button" title={tr('cfg.subir')} onClick={() => mudarFase(i, { etapas: mover(f.etapas, j, -1) })} disabled={j === 0} style={{ ...mini, opacity: j === 0 ? 0.35 : 1 }}>↑</button>
                          <button type="button" title={tr('cfg.descer')} onClick={() => mudarFase(i, { etapas: mover(f.etapas, j, 1) })} disabled={j === f.etapas.length - 1} style={{ ...mini, opacity: j === f.etapas.length - 1 ? 0.35 : 1 }}>↓</button>
                          <button type="button" title={tr('cfg.remover-etapa')} onClick={() => tirarEtapa(i, j)} style={{ ...mini, color: 'var(--v2-hot)' }}>×</button>
                        </div>
                      </div>
                    ))}
                    <button type="button" onClick={() => addEtapa(i)} style={{ alignSelf: 'flex-start', padding: '8px 12px', borderRadius: 10, border: '1px dashed var(--v2-rule2)', background: 'none', color: 'var(--v2-ink2)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, minHeight: 36 }}>{tr('cfg.add-etapa')}</button>
                  </div>
                )}
              </div>
            )
          })}
          <button type="button" onClick={addFase} style={{ alignSelf: 'flex-start', padding: '9px 14px', borderRadius: 10, border: '1px dashed var(--v2-rule2)', background: 'none', color: 'var(--v2-ink2)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13.5, minHeight: 38 }}>{tr('cfg.add-fase')}</button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 12.5, color: 'var(--v2-ink3)' }}>{cfg.fases.length} {tr(cfg.fases.length === 1 ? 'cfg.fase' : 'cfg.fases')} · {totalEtapas} {tr(totalEtapas === 1 ? 'cfg.etapa' : 'cfg.etapas')}</span>
        <button type="button" onClick={restaurarPadrao} style={{ marginLeft: 'auto', background: 'none', border: 0, color: 'var(--v2-ink3)', fontSize: 12.5, cursor: 'pointer', fontFamily: 'inherit' }}>{tr('cfg.restaurar')}</button>
      </div>
    </div>
  )
}
