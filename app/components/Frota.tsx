...
const CONDICOES: { key: Condicao; label: string; cor: string; bg: string }[] = [
  { key: 'disponivel', label: 'Disponível para viajar', cor: '#166534', bg: '#dcfce7' },
  { key: 'ocupado', label: 'Ocupado', cor: '#1d4ed8', bg: '#eff6ff' },
  { key: 'manutencao', label: 'Em manutenção', cor: '#a16207', bg: '#fef9c3' },
  { key: 'excluido', label: 'Excluído', cor: '#9ca3af', bg: '#f4f4f5' },
]
...
export default function Frota({ podeEditar = true, podeExcluir = false }: { podeEditar?: boolean; podeExcluir?: boolean }) {
  ...
  const btnSecao = (k: typeof secao, label: string, badge?: number) => (
    <button type="button" onClick={() => setSecao(k)}
      style={{ padding: '7px 13px', borderRadius: 9, border: 'none', background: secao === k ? '#111' : 'transparent', color: secao === k ? '#fff' : '#777', fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}>
      {label}{badge ? ` (${badge})` : ''}
    </button>
  )
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <h2 style={{ margin: 0, fontSize: 20, color: '#111' }}>Frota</h2>
        <span style={{ flex: 1 }} />
        {excluidos > 0 && (
          <button onClick={() => setVerExcluidos(v => !v)} style={{ padding: '7px 12px', background: 'transparent', border: '1px solid #e6e6e6', borderRadius: 9, color: '#777', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
            {verExcluidos ? 'Ocultar excluídos' : `Ver excluídos (${excluidos})`}
          </button>
        )}
        {podeEditar && <button onClick={abrirNovo} style={{ padding: '9px 16px', background: '#111', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>+ Veículo</button>}
      </div>
      <p style={{ margin: '0 0 16px', fontSize: 13, color: '#999' }}>Veículos da operadora. O croqui de cada um define o mapa de poltronas usado nas reservas — e a capacidade sai dele.</p>

      {carregando ? <p style={{ color: '#aaa', fontSize: 13 }}>Carregando...</p>
        : visiveis.length === 0 ? <p style={{ color: '#aaa', fontSize: 13 }}>Nenhum veículo cadastrado.</p>
        : (
            ...
            {visiveis.map(v => {
              ...
              return (
                <div key={v.id} onClick={() => podeEditar && abrirEditar(v)}
                  style={{ background: '#fff', border: '1px solid #eee', borderRadius: 12, padding: 14, boxShadow: '0 1px 4px rgba(0,0,0,0.05)', cursor: podeEditar ? 'pointer' : 'default', opacity: v.condicao === 'excluido' ? 0.6 : 1 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: '#111' }}>{v.nome}</span>
                    {v.placa && <span style={{ fontSize: 11.5, color: '#999', fontWeight: 600 }}>{v.placa}</span>}
                    <span style={{ flex: 1 }} />
                  ...
                  {v.amenidades && v.amenidades.length > 0 && (
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 8 }}>
                      {v.amenidades.map(a => <span key={a} style={{ fontSize: 10.5, fontWeight: 600, color: '#1d4ed8', background: '#eff6ff', borderRadius: 999, padding: '2px 8px' }}>{a}</span>)}
                    </div>
                  )}
                </div>
              )
      ...
      {form && (
        <div onClick={fecharFora(() => setForm(null))} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, maxWidth: 760, width: '100%', maxHeight: '92vh', overflowY: 'auto', padding: 22 }}>
            ...
            <div style={{ display: 'flex', gap: 4, marginBottom: 16, background: '#f6f6f6', borderRadius: 10, padding: 3, width: 'fit-content' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: 140 }}>
                    ...
                    <select value={form.tipo} onChange={e => setForm(f => f && ({ ...f, tipo: e.target.value as TipoVeiculo }))} style={{ ...inputStyle, width: '100%', background: '#fff' }}>
                      {TIPOS_VEICULO.map(t => <option key={t.key} value={t.key}>{t.label}</option>)}
                    </select>
                  </div>
                  <div style={{ flex: 1, minWidth: 140 }}>
                    <label style={labelStyle}>Capacidade</label>
                    <div style={{ ...inputStyle, background: '#f8fafc', color: '#666', display: 'flex', alignItems: 'center', gap: 8 }}>
                      <strong style={{ color: '#111' }}>{capacidadeLayout(form.layout)}</strong>
                      <span style={{ fontSize: 11.5 }}>poltronas</span>
                      <span style={{ flex: 1 }} />
                      <button type="button" onClick={() => setSecao('croqui')} style={{ background: 'none', border: 'none', color: '#2563eb', fontSize: 11.5, fontWeight: 700, cursor: 'pointer', padding: 0 }}>Editar croqui</button>
                    </div>
                  </div>
                  <div style={{ flex: 1, minWidth: 170 }}>
                    <label style={labelStyle}>Condição</label>
                    <select value={form.condicao} onChange={e => setForm(f => f && ({ ...f, condicao: e.target.value as Condicao }))} style={{ ...inputStyle, width: '100%', background: '#fff' }}>
                      {CONDICOES.map(c => <option key={c.key} value={c.key}>{c.label}</option>)}
                    </select>
                  </div>
                </div>
                <p style={{ margin: '-4px 0 0', fontSize: 11, color: '#bbb' }}>A capacidade é contada do croqui — assim ela nunca diverge do mapa de poltronas das reservas. Só &quot;Disponível para viajar&quot; aparece na hora de montar a viagem.</p>
                <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <span style={{ fontSize: 13, fontWeight: 800, color: '#111' }}>Documentos com validade</span>
                    <span style={{ flex: 1 }} />
                    <button type="button" onClick={addDoc} style={{ background: 'none', border: 'none', color: '#2563eb', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ Documento</button>
                  </div>
                  <p style={{ margin: '0 0 8px', fontSize: 11, color: '#bbb' }}>Vencendo em até 30 dias vira tarefa para a equipe automaticamente.</p>
                  {form.documentos.length === 0 && <p style={{ margin: 0, fontSize: 12, color: '#ccc' }}>Nenhum documento cadastrado.</p>}
                  {form.documentos.map(d => {
                    ...
                    return (
                      <div key={d.id} style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                        <select value={d.tipo} onChange={e => setDoc(d.id, { tipo: e.target.value })} style={{ ...inputStyle, background: '#fff', flex: 1, minWidth: 130 }}>
                        ...
                        {dias !== null && dias <= 30 && (
                          <span style={{ fontSize: 10.5, fontWeight: 800, color: dias < 0 ? '#b91c1c' : '#a16207', background: dias < 0 ? '#fee2e2' : '#fef9c3', borderRadius: 999, padding: '3px 8px' }}>
                            {dias < 0 ? 'vencido' : dias === 0 ? 'vence hoje' : `${dias}d`}
                          </span>
                        )}
                        <button type="button" onClick={() => rmDoc(d.id)} style={{ background: 'none', border: 'none', color: '#ccc', cursor: 'pointer', fontSize: 18 }}>×</button>
                      </div>
                    )
                  })}
                </div>
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <span style={{ fontSize: 13, fontWeight: 800, color: '#111' }}>Histórico de serviços</span>
                    {form.manutencoes.length > 0 && <span style={{ fontSize: 11.5, color: '#666' }}>· total {fmtBRL(form.manutencoes.reduce((s, m) => s + (Number(m.custo) || 0), 0))}</span>}
                    <span style={{ flex: 1 }} />
                    <button type="button" onClick={addManut} style={{ background: 'none', border: 'none', color: '#2563eb', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ Serviço</button>
                  </div>
                  {form.manutencoes.length === 0 && <p style={{ margin: 0, fontSize: 12, color: '#ccc' }}>Nenhum serviço registrado.</p>}
                  {form.manutencoes.map(m => (
                    <div key={m.id} style={{ border: '1px solid #eee', borderRadius: 10, padding: 10, marginBottom: 8 }}>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 6 }}>
                        <input type="date" value={m.data} onChange={e => setManut(m.id, { data: e.target.value })} style={{ ...inputStyle, minWidth: 140 }} />
                        <select value={m.tipo} onChange={e => setManut(m.id, { tipo: e.target.value })} style={{ ...inputStyle, background: '#fff', flex: 1, minWidth: 120 }}>
                        ...
                        <button type="button" onClick={() => rmManut(m.id)} style={{ background: 'none', border: 'none', color: '#ccc', cursor: 'pointer', fontSize: 18, marginLeft: 'auto' }}>×</button>
                      </div>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 6, alignItems: 'center' }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: '#888' }}>Próxima revisão:</span>
                        ...
                        {m.proximaData && diasAte(m.proximaData) <= 30 && (
                          <span style={{ fontSize: 10.5, fontWeight: 800, color: diasAte(m.proximaData) < 0 ? '#b91c1c' : '#a16207', background: diasAte(m.proximaData) < 0 ? '#fee2e2' : '#fef9c3', borderRadius: 999, padding: '3px 8px' }}>
