    ...
    <div style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr' : 'minmax(0, 1.3fr) minmax(280px, 1fr)', gap: mobile ? 16 : 24, alignItems: 'start' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                return (
                  <button key={conta.id} type="button" onClick={() => alternarConta(conta.id)}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '9px 13px', borderRadius: 10, cursor: 'pointer', fontFamily: 'inherit',
                      border: `1.5px solid ${ativo ? '#1877f2' : 'var(--v2-rule)'}`, background: ativo ? '#1877f210' : 'var(--v2-surface)' }}>
                    <span style={{ width: 16, height: 16, borderRadius: 5, border: `1.5px solid ${ativo ? '#1877f2' : 'var(--v2-rule2)'}`, background: ativo ? '#1877f2' : 'var(--v2-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {ativo && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--v2-surface)" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5" /></svg>}
                    </span>
                    <span style={{ width: 22, height: 22, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, background: 'var(--marca, var(--v2-amber-on))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: 'var(--v2-ink)' }}>
                      <AvatarCliente logo={conta.logo} nome={conta.nome} />
                    </span>
                    <span style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.2, textAlign: 'left' }}>
                      <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--v2-ink)' }}>{conta.nome}</span>
                      {redesConta && <span style={{ fontSize: 10.5, color: 'var(--v2-ink3)' }}>{redesConta}</span>}
                    </span>
                  </button>
                )
              })}
              ...
              return (
                <button key={r.key} type="button" onClick={() => alternarRede(r.key)}
                  style={{
                    ...
                    border: `1.5px solid ${ativo ? sel : 'var(--v2-rule)'}`, background: ativo ? `${sel}10` : 'var(--v2-surface)', fontFamily: 'inherit',
                  }}>
                  <span style={{
                    width: 18, height: 18, borderRadius: 5, border: `1.5px solid ${ativo ? sel : 'var(--v2-rule2)'}`, background: ativo ? sel : 'var(--v2-surface)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--v2-surface)', fontSize: 12, flexShrink: 0,
                  }}>{ativo ? <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--v2-surface)" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5" /></svg> : null}</span>
                  <span style={{ width: 22, height: 22, borderRadius: '50%', background: r.cor, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="var(--v2-surface)">{r.key === 'facebook'
                  ...
                  <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--v2-ink)' }}>{r.nome}</span>
                </button>
              )
            })}
          </div>
        ...
        <div>
          ...
          <div
            ...
            style={{
              border: `1.5px dashed ${arrastando ? 'var(--v2-amber-on)' : 'var(--v2-rule)'}`,
              borderRadius: 12, padding: '24px 16px', textAlign: 'center', cursor: 'pointer',
              background: arrastando ? 'var(--v2-amber-bg)' : 'var(--v2-surface1)', transition: 'all .15s',
            }}
          >
          ...
          {emEnvio.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12 }}>
              {emEnvio.map(e => (
                <div key={e.id}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--v2-ink3)', marginBottom: 4 }}>
                  ...
                  <div style={{ height: 6, borderRadius: 999, background: 'var(--v2-surface2)', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${e.progresso}%`, background: 'var(--v2-amber-on)', transition: 'width .2s' }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {erroUpload && (
            <p style={{ margin: '8px 0 0', fontSize: 12, color: 'var(--v2-hot)' }}>{erroUpload}</p>
          )}

          {midias.length > 1 && (
            <p style={{ margin: '12px 0 0', fontSize: 11, color: 'var(--v2-ink3)' }}>Arraste as mídias para reordenar o carrossel (a ordem abaixo é a ordem da publicação).</p>
          )}
          {midias.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 8 }}>
              {midias.map((m, i) => (
                <div key={i}
                  ...
                  style={{
                    position: 'relative', width: 84, height: 84, borderRadius: 10, overflow: 'hidden', background: 'var(--v2-surface2)',
                    cursor: midias.length > 1 ? 'grab' : 'default',
                    opacity: dragMidia === i ? 0.4 : 1,
                    outline: overMidia === i && dragMidia !== i ? '2px solid var(--v2-amber-on)' : 'none', outlineOffset: -2,
                  }}>
                  {m.tipo === 'video' ? (
                    m.capa
                  ...
                  <button onClick={(e) => { e.stopPropagation(); removerMidia(i) }} style={{
                    ...
                    background: 'rgba(0,0,0,0.6)', color: 'var(--v2-surface)', border: 'none', cursor: 'pointer', fontSize: 13, lineHeight: 1,
                  }}>×</button>
                  {m.tipo === 'video' && (
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {FORMATOS.map(f => (
              <button key={f.key} onClick={() => setFormato(f.key)} type="button" style={{
                ...
                borderColor: formato === f.key ? 'var(--v2-ink)' : 'var(--v2-rule)',
                background: formato === f.key ? 'var(--v2-ink)' : 'var(--v2-surface)',
                color: formato === f.key ? 'var(--v2-amber-on)' : 'var(--v2-ink3)',
              }}>
                {f.label}
              </button>
            ))}
        ...
        <div>
          ...
          {colaboradores.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
              {colaboradores.map(c => (
                <span key={c} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--v2-ink)', color: 'var(--v2-amber-on)', borderRadius: 999, padding: '5px 6px 5px 12px', fontSize: 13, fontWeight: 700 }}>
                  @{c}
                  <button type="button" onClick={() => removerColab(c)} style={{ width: 18, height: 18, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', color: 'var(--v2-surface)', border: 'none', cursor: 'pointer', fontSize: 12, lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
                </span>
              ))}
            </div>
          )}
            ...
            <div style={{ position: 'relative' }}>
              ...
              {colabResultado && (
                <div onClick={() => adicionarColab(colabResultado.username)}
                  style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: 'var(--v2-surface)', border: '1.5px solid var(--v2-rule)', borderRadius: 10, padding: 10, display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', boxShadow: '0 4px 14px rgba(0,0,0,0.08)', zIndex: 5 }}>
                  {colabResultado.foto
                    ? <img src={colabResultado.foto} alt="" style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
                    : <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--v2-surface2)', flexShrink: 0 }} />}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: 'var(--v2-ink)' }}>@{colabResultado.username}</p>
                    <p style={{ margin: 0, fontSize: 11, color: 'var(--v2-ink3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {colabResultado.nome}{colabResultado.seguidores != null ? ` · ${colabResultado.seguidores.toLocaleString('pt-BR')} seguidores` : ''}
                    </p>
                  </div>
                  <span style={{ fontSize: 11, color: 'var(--v2-ink3)', flexShrink: 0 }}>Enter</span>
                </div>
              )}
              ...
              {!colabResultado && !colabBuscando && /^[A-Za-z0-9._]{2,30}$/.test(colabBusca.trim().replace(/^@/, '')) && (
                <div onClick={() => adicionarColab(colabBusca)}
                  style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: 'var(--v2-surface)', border: '1.5px solid var(--v2-rule)', borderRadius: 10, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', boxShadow: '0 4px 14px rgba(0,0,0,0.08)', zIndex: 5 }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--v2-surface2)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--v2-ink3)', fontWeight: 800 }}>@</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: 'var(--v2-ink)' }}>Adicionar @{colabBusca.trim().replace(/^@/, '')}</p>
                    <p style={{ margin: 0, fontSize: 11, color: 'var(--v2-ink3)' }}>{colabErro ? colabErro : 'Perfil não verificado pela API — será marcado mesmo assim.'}</p>
                  </div>
                  <span style={{ fontSize: 11, color: 'var(--v2-ink3)', flexShrink: 0 }}>Enter</span>
                </div>
              )}
      ...
      <div style={{ position: 'sticky', top: 16 }}>
        ...
        <div style={{ background: 'var(--v2-surface)', border: '1px solid var(--v2-rule)', borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px' }}>
            <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--v2-amber-on)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 12, color: '#17150E', flexShrink: 0, overflow: 'hidden' }}>
              ...
              <div style={{
                ...
                {midias.length > 1 && (
                  <>
                    ...
                    {/* Contador 1/N */}
                    <div style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(0,0,0,0.55)', color: 'var(--v2-surface)', fontSize: 11, fontWeight: 700, borderRadius: 999, padding: '2px 8px' }}>{idx + 1}/{midias.length}</div>
                    {/* Bolinhas */}
                    <div style={{ position: 'absolute', bottom: 8, left: 0, right: 0, display: 'flex', justifyContent: 'center', gap: 5 }}>
                      {midias.map((_, i) => (
                        <span key={i} onClick={() => setPreviewIdx(i)} style={{ width: 6, height: 6, borderRadius: '50%', background: i === idx ? 'var(--v2-surface)' : 'rgba(255,255,255,0.5)', boxShadow: '0 0 2px rgba(0,0,0,0.4)', cursor: 'pointer' }} />
                      ))}
                    </div>
                  </>
                )}
      ...
      {frameModal && (
        <div onClick={() => !capturandoFrame && setFrameModal(null)}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--v2-surface)', borderRadius: 16, padding: 18, width: '100%', maxWidth: 480 }}>
            <h3 style={{ margin: '0 0 4px', fontSize: 15, fontWeight: 700, color: 'var(--v2-ink)' }}>Escolher capa do vídeo</h3>
            <p style={{ margin: '0 0 12px', fontSize: 12, color: 'var(--v2-ink3)' }}>Dê play e pause no momento que quiser — depois clique em "Usar este frame".</p>
            <video ref={videoFrameRef} src={`${frameModal.url}${frameModal.url.includes('?') ? '&' : '?'}cors=1`} crossOrigin="anonymous" controls playsInline muted
              style={{ width: '100%', borderRadius: 10, background: 'var(--v2-ink)', maxHeight: '60vh' }} />
            {erroUpload && <p style={{ margin: '10px 0 0', fontSize: 12.5, color: 'var(--v2-hot)', fontWeight: 600 }}>{erroUpload}</p>}
            <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
              <button onClick={() => setFrameModal(null)} disabled={capturandoFrame}
                style={{ flex: 1, padding: '12px 0', background: 'var(--v2-surface)', color: 'var(--v2-ink2)', border: '1.5px solid var(--v2-rule)', borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
                Cancelar
              </button>
              <button onClick={capturarFrameDoVideo} disabled={capturandoFrame}
                style={{ flex: 1.4, padding: '12px 0', background: 'var(--v2-amber-on)', border: 'none', borderRadius: 10, fontWeight: 800, fontSize: 14, cursor: 'pointer', opacity: capturandoFrame ? 0.6 : 1 }}>
