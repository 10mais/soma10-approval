  ...
  return (
    <div style={{ maxWidth: 860 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 6 }}>
        ...
        <button onClick={() => setEdit(novaRegra())} style={{ padding: '9px 16px', background: '#ffc00f', color: '#111', border: 'none', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>+ Nova automação</button>
      </div>
      <p style={{ margin: '0 0 20px', fontSize: 13, color: '#999' }}>Quando o <b>gatilho</b> acontecer (e as condições baterem), executa a <b>sequência</b> de passos. Global, com exceção por cliente.</p>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {regras.length === 0 && <p style={{ fontSize: 13, color: '#bbb' }}>Nenhuma automação ainda. Crie a primeira.</p>}
        {regras.map(r => (
          <div key={r.id} style={{ background: '#fff', borderRadius: 14, padding: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: '#111' }}>{r.nome}</p>
            ...
            <button onClick={() => setEdit(JSON.parse(JSON.stringify(r)))} style={{ background: 'none', border: '1px solid #e0e0e0', borderRadius: 8, padding: '6px 12px', fontSize: 12, color: '#666', cursor: 'pointer' }}>Editar</button>
            <button onClick={() => excluir(r)} style={{ background: 'none', border: '1px solid #fecaca', borderRadius: 8, padding: '6px 12px', fontSize: 12, color: '#ef4444', cursor: 'pointer' }}>Excluir</button>
            <button onClick={() => toggleAtivo(r)} title={r.ativo ? 'Ativa' : 'Inativa'} style={{ flexShrink: 0, width: 46, height: 26, borderRadius: 999, border: 'none', cursor: 'pointer', background: r.ativo ? '#16a34a' : '#e0e0e0', position: 'relative' }}>
              <span style={{ position: 'absolute', top: 3, left: r.ativo ? 23 : 3, width: 20, height: 20, borderRadius: '50%', background: '#fff', boxShadow: '0 1px 3px rgba(0,0,0,0.25)', transition: 'left .2s' }} />
            </button>
          </div>
        ))}
      </div>
...
const inp: React.CSSProperties = { padding: '8px 10px', borderRadius: 8, border: '1px solid #e0e0e0', fontSize: 13, fontFamily: 'inherit', boxSizing: 'border-box' }
const lbl: React.CSSProperties = { display: 'block', fontSize: 11, fontWeight: 700, color: '#888', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.04em' }
    ...
    <div onClick={fecharFora(onClose)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000, padding: 20 }}>
      <div onClick={e => e.stopPropagation()} className="soma10-no-invert" style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 620, maxHeight: '92vh', overflowY: 'auto', padding: 22 }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <select value={regra.gatilho} onChange={e => set({ gatilho: e.target.value, condicoes: [] })} style={{ ...inp, width: '100%', background: '#fff' }}>
        ...
        {/* Condições */}
        {regra.gatilho && (
          <div style={{ marginBottom: 14, background: '#fafafa', borderRadius: 10, padding: 12 }}>
            ...
            {(regra.condicoes || []).map((c: Cond, i: number) => (
              <div key={i} style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'center' }}>
                <select value={c.campo} onChange={e => { const cs = [...regra.condicoes]; cs[i] = { ...c, campo: e.target.value }; set({ condicoes: cs }) }} style={{ ...inp, flex: 1, background: '#fff' }}>
                  <option value="">Campo…</option>
                  {campos.map((f: any) => <option key={f.chave} value={f.chave}>{f.label}</option>)}
                </select>
                <select value={c.operador} onChange={e => { const cs = [...regra.condicoes]; cs[i] = { ...c, operador: e.target.value }; set({ condicoes: cs }) }} style={{ ...inp, background: '#fff' }}>
                ...
                <button onClick={() => set({ condicoes: regra.condicoes.filter((_: any, j: number) => j !== i) })} style={{ background: 'none', border: 'none', color: '#ccc', cursor: 'pointer', fontSize: 16 }}>×</button>
              </div>
            ))}
            <button onClick={() => set({ condicoes: [...(regra.condicoes || []), { campo: '', operador: 'igual', valor: '' }] })} style={{ background: 'none', border: 'none', color: '#1d4ed8', fontSize: 12, fontWeight: 700, cursor: 'pointer', padding: 0 }}>+ Condição</button>
          </div>
        )}
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            {(['todos', 'selecionados'] as const).map(a => (
              <button key={a} onClick={() => set({ alvo: a })} style={{ padding: '7px 14px', borderRadius: 8, border: regra.alvo === a ? '1.5px solid #111' : '1.5px solid #e0e0e0', background: regra.alvo === a ? '#111' : '#fff', color: regra.alvo === a ? '#fff' : '#666', fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}>{a === 'todos' ? 'Todos os clientes' : 'Clientes específicos'}</button>
            ))}
          </div>
        ...
        <div style={{ marginBottom: 16 }}>
            ...
            return (
              <div key={p.id} style={{ background: '#fafafa', borderRadius: 10, padding: 12, marginBottom: 8 }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: ac ? 10 : 0, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: '#888' }}>Passo {i + 1}</span>
                  <span style={{ fontSize: 12, color: '#888' }}>após</span>
                  ...
                  <select value={p.acao} onChange={e => { const ps = [...regra.passos]; ps[i] = { ...p, acao: e.target.value, params: {} }; set({ passos: ps }) }} style={{ ...inp, flex: 1, minWidth: 160, background: '#fff' }}>
                    <option value="">Ação…</option>
                    {ACOES.map(a => <option key={a.chave} value={a.chave}>{a.label}</option>)}
                  </select>
                  {regra.passos.length > 1 && <button onClick={() => set({ passos: regra.passos.filter((_: any, j: number) => j !== i) })} style={{ background: 'none', border: 'none', color: '#ccc', cursor: 'pointer', fontSize: 16 }}>×</button>}
                </div>
                {ac && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                    {ac.params.map(par => (
                      <div key={par.chave}>
                        <label style={{ fontSize: 11, color: '#999', display: 'block', marginBottom: 2 }}>{par.label}{par.dica ? ` — ${par.dica}` : ''}</label>
                        {par.tipo === 'select' ? (
                          <select value={p.params[par.chave] || ''} onChange={e => { const ps = [...regra.passos]; ps[i] = { ...p, params: { ...p.params, [par.chave]: e.target.value } }; set({ passos: ps }) }} style={{ ...inp, width: '100%', background: '#fff' }}>
                            <option value="">—</option>
          ...
          <button onClick={() => set({ passos: [...regra.passos, { id: uid(), atrasoDias: 0, acao: '', params: {} }] })} style={{ background: 'none', border: '1px dashed #ccc', borderRadius: 8, padding: '8px 12px', fontSize: 12.5, fontWeight: 700, color: '#666', cursor: 'pointer', width: '100%' }}>+ Adicionar passo</button>
        </div>

        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ padding: '10px 18px', background: '#f0f0f0', color: '#666', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>Cancelar</button>
          <button onClick={salvar} disabled={salvando} style={{ padding: '10px 20px', background: '#111', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>{salvando ? 'Salvando…' : 'Salvar automação'}</button>
        </div>
      </div>
    </div>
  )
...
function ClienteMulti({ clientes, sel, onChange, label }: { clientes: { id: string; nome: string }[]; sel: string[]; onChange: (ids: string[]) => void; label: string }) {
  return (
    <div>
      <p style={{ margin: '0 0 6px', fontSize: 11, color: '#999' }}>{label}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, maxHeight: 120, overflowY: 'auto' }}>
        {clientes.length === 0 && <span style={{ fontSize: 12, color: '#bbb' }}>Sem clientes.</span>}
        {clientes.map(c => {
          const on = sel.includes(c.id)
          return (
            <button key={c.id} onClick={() => onChange(on ? sel.filter(x => x !== c.id) : [...sel, c.id])} style={{ padding: '5px 11px', borderRadius: 999, border: on ? '1.5px solid #1d4ed8' : '1.5px solid #e0e0e0', background: on ? '#eff6ff' : '#fff', color: on ? '#1d4ed8' : '#888', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>{c.nome}</button>
          )
        })}
      </div>
    </div>
