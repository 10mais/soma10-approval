...
const ESTILO_DOC = `.doc-conteudo a{color:var(--v2-info);text-decoration:underline}
...
.doc-conteudo blockquote{border-left:3px solid var(--v2-rule);margin:.5em 0;padding:.1em 0 .1em .9em;color:var(--v2-ink2)}
...
export default function DocumentosDoCliente() {
  ...
  if (carregando) return <p style={{ color: 'var(--v2-ink3)', fontSize: 13 }}>Carregando…</p>
    ...
    return (
      <div style={{ maxWidth: 820 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
          <button onClick={() => setAberto(null)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: 'var(--v2-surface)', color: 'var(--v2-ink2)', border: '1px solid var(--v2-rule)', borderRadius: 10, fontWeight: 600, fontSize: 12.5, cursor: 'pointer' }}>
        ...
        <div style={{ background: 'var(--v2-surface)', borderRadius: 16, boxShadow: '0 2px 10px rgba(0,0,0,0.06)', padding: '28px 30px' }}>
          {podeEditar ? (
            <>
              <input value={aberto.titulo} onChange={e => editar({ titulo: e.target.value })} placeholder="Título do documento"
                style={{ width: '100%', border: 'none', outline: 'none', fontSize: 22, fontWeight: 800, color: 'var(--v2-ink)', fontFamily: 'inherit', background: 'transparent', marginBottom: 12, boxSizing: 'border-box' }} />
            ...
            <>
              <h1 style={{ margin: '0 0 6px', fontSize: 24, fontWeight: 800, color: 'var(--v2-ink)' }}>{aberto.titulo?.trim() || 'Documento'}</h1>
              <p style={{ margin: '0 0 20px', fontSize: 12, color: 'var(--v2-ink3)' }}>Atualizado em {new Date(aberto.atualizadoEm).toLocaleDateString('pt-BR')}{aberto.atualizadoPorNome ? ` por ${aberto.atualizadoPorNome}` : ''}</p>
              <div className="doc-conteudo" style={{ fontSize: aberto.fontSize || 15, lineHeight: 1.65, color: 'var(--v2-ink)', wordBreak: 'break-word' }}
                dangerouslySetInnerHTML={{ __html: aberto.conteudo ? sanitizeHtml(aberto.conteudo) : '<p style="color:var(--v2-ink3)">Documento vazio.</p>' }} />
            </>
          )}
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth: 820 }}>
      <div style={{ marginBottom: 16 }}>
        ...
        <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--v2-ink3)' }}>Documentos que a equipe compartilhou com você. Alguns podem ser editados por você.</p>
      </div>
      {docs.length === 0 ? (
        <div style={{ background: 'var(--v2-surface)', borderRadius: 14, padding: '48px 20px', textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <p style={{ margin: 0, fontSize: 13.5, color: 'var(--v2-ink3)' }}>Nenhum documento compartilhado com você por enquanto.</p>
        </div>
      ) : (
          ...
          {docs.map((d, idx) => (
            <div key={d.id} onClick={() => setAberto(d)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderTop: idx ? '1px solid var(--v2-surface1)' : 'none', cursor: 'pointer' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ margin: '2px 0 0', fontSize: 12, color: 'var(--v2-ink3)' }}>{d.acessoCliente === 'editar' ? 'Você pode editar' : 'Somente leitura'} · atualizado {quando(d.atualizadoEm)}</p>
              </div>
