...
function prazoMarco(m: Marco): { texto: string; cor: string } | null {
  ...
  if (m.status === 'concluido') {
    ...
    return diasAtraso <= 0 ? { texto: 'Entregue no prazo', cor: 'var(--v2-ok)' } : { texto: `Entregue com ${diasAtraso}d de atraso`, cor: 'var(--v2-hot)' }
  }
  if (m.status !== 'cancelado' && prazo < Date.now()) {
    const dias = Math.round((Date.now() - prazo) / 86400000)
    return { texto: `Atrasada ${dias}d`, cor: 'var(--v2-hot)' }
  }
  return null
}
...
const STATUS_COR: Record<string, string> = { planejado: 'var(--v2-ink3)', em_andamento: 'var(--v2-amber)', concluido: 'var(--v2-ok)', atrasado: 'var(--v2-hot)', cancelado: 'var(--v2-ink3)' }
...
export default function EntregasPage() {
  ...
  const [cor, setCor] = useState('var(--v2-ok)')
  ...
  if (carregando) return <div style={{ padding: 60, textAlign: 'center', color: 'var(--v2-ink3)' }}>Carregando...</div>

  return (
    <div style={{ maxWidth: 820 }}>
      <h2 style={{ margin: '0 0 4px', fontSize: 18, color: 'var(--v2-ink)' }}>Entregas</h2>
      <p style={{ margin: '0 0 20px', fontSize: 13, color: 'var(--v2-ink3)' }}>Acompanhe a evolução do seu projeto — o que foi contratado, o que já foi entregue e cada etapa da jornada.</p>

      {/* Onboarding — só para a equipe (início da jornada do cliente). Some quando completo. */}
      {ehEquipe && !onboardingCompleto && (
        <div style={{ background: 'var(--v2-surface)', border: '1.5px solid var(--v2-amber-bg)', borderRadius: 14, padding: 18, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--v2-ink)' }}>Onboarding do cliente <span style={{ color: 'var(--v2-ink3)', fontWeight: 600 }}>(visível só para a equipe)</span></span>
            <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--v2-amber)' }}>{onboardingFeitos}/{onboarding.length}</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {onboarding.map((o, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {o.ok ? (
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--v2-ok)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M20 6L9 17l-5-5" /></svg>
                ) : (
                  <span style={{ width: 18, height: 18, borderRadius: '50%', border: '2px solid var(--v2-rule)', flexShrink: 0 }} />
                )}
                <span style={{ fontSize: 13, color: o.ok ? 'var(--v2-ink3)' : 'var(--v2-ink)', textDecoration: o.ok ? 'line-through' : 'none', flex: 1 }}>{o.label}</span>
                {!o.ok && <span style={{ fontSize: 11, color: 'var(--v2-ink3)' }}>{o.dica}</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      {(entregaveis.length > 0 || postsContratados > 0 || ehEquipe) && (
        <div style={{ background: 'var(--v2-surface)', borderRadius: 14, padding: 18, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: 18 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--v2-ink)' }}>Escopo do contrato</span>
          ...
          {(postsContratados > 0 || ehEquipe) && (
            <div style={{ marginTop: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6, gap: 8 }}>
                <span style={{ fontSize: 12.5, color: 'var(--v2-ink2)', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                  Meta de posts do mês
                  {ehEquipe && !editandoMeta && (
                    <button onClick={() => { setMetaInput(String(postsContratados)); setEditandoMeta(true) }} title="Editar meta do mês" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--v2-info)', fontSize: 11, fontWeight: 700, padding: 0 }}>editar</button>
                  )}
                </span>
              ...
              {editandoMeta ? (
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 2 }}>
                  <input type="number" min="0" autoFocus value={metaInput} onChange={e => setMetaInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') salvarMeta() }}
                    style={{ width: 90, padding: '8px 10px', borderRadius: 8, border: '1.5px solid var(--v2-rule)', fontSize: 13, fontFamily: 'inherit' }} />
                  <span style={{ fontSize: 12, color: 'var(--v2-ink3)' }}>posts/mês</span>
                  <button onClick={salvarMeta} disabled={salvandoMeta} style={{ padding: '8px 14px', background: 'var(--v2-ink)', color: 'var(--v2-surface)', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{salvandoMeta ? '...' : 'Salvar'}</button>
                  <button onClick={() => setEditandoMeta(false)} style={{ padding: '8px 10px', background: 'var(--v2-surface)', color: 'var(--v2-ink2)', border: '1.5px solid var(--v2-rule)', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>Cancelar</button>
                </div>
              ) : postsContratados > 0 ? (
                <>
                  <div style={{ height: 8, borderRadius: 999, background: 'var(--v2-surface2)', overflow: 'hidden' }}>
                    <div style={{ width: `${pctMes}%`, height: '100%', background: cor, borderRadius: 999, transition: 'width .3s' }} />
                  </div>
                  <p style={{ margin: '6px 0 0', fontSize: 11.5, color: 'var(--v2-ink3)' }}>{postsPublicadosMes >= postsContratados ? 'Meta do mês atingida!' : `Faltam ${postsContratados - postsPublicadosMes} post(s) para o combinado do mês.`}</p>
                  {postsPublicadosMes < postsContratados && diaDoMes >= 3 && (
                    emRisco ? (
                      <p style={{ margin: '6px 0 0', fontSize: 11.5, fontWeight: 700, color: 'var(--v2-hot)', background: 'var(--v2-hot-bg)', borderRadius: 8, padding: '6px 10px' }}>
                      ...
                      <p style={{ margin: '6px 0 0', fontSize: 11.5, color: 'var(--v2-ok)' }}>No ritmo certo para bater a meta do mês.</p>
                    )
                  )}
                </>
              ) : (
                <p style={{ margin: '2px 0 0', fontSize: 11.5, color: 'var(--v2-ink3)' }}>Defina a meta mensal de posts deste cliente (clique em “editar”).</p>
              )}
            </div>
          )}
        </div>
      ...
      {totalItens > 0 && (
        <div style={{ background: 'var(--v2-surface)', borderRadius: 14, padding: 18, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--v2-ink)' }}>Progresso geral</span>
            <span style={{ fontSize: 16, fontWeight: 800, color: cor }}>{pctGeral}%</span>
          </div>
          <div style={{ height: 10, borderRadius: 999, background: 'var(--v2-surface2)', overflow: 'hidden' }}>
            <div style={{ width: `${pctGeral}%`, height: '100%', background: cor, borderRadius: 999, transition: 'width .3s' }} />
          </div>
          <p style={{ margin: '8px 0 0', fontSize: 12, color: 'var(--v2-ink3)' }}>{feitos} de {totalItens} entregas concluídas · {marcos.length} etapa(s)</p>
          {(() => {
            ...
            return <p style={{ margin: '4px 0 0', fontSize: 12, color: noPrazo === comPrazo.length ? 'var(--v2-ok)' : 'var(--v2-amber)' }}>SLA de entrega: {noPrazo} de {comPrazo.length} etapa(s) no prazo.</p>
          })()}
        </div>
      )}
      ...
      {ordenados.length === 0 ? (
        <div style={{ background: 'var(--v2-surface)', borderRadius: 14, padding: '50px 20px', textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <p style={{ margin: 0, fontSize: 14, color: 'var(--v2-ink3)' }}>Ainda não há etapas cadastradas.</p>
        </div>
      ) : (
          ...
          {ordenados.map((m, i) => (
            <div key={m.id} style={{ display: 'flex', gap: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                <div style={{ width: 14, height: 14, borderRadius: '50%', background: STATUS_COR[m.status] || 'var(--v2-rule)', border: '2px solid var(--v2-surface)', boxShadow: '0 0 0 2px #eee', marginTop: 22 }} />
                {i < ordenados.length - 1 && <div style={{ flex: 1, width: 2, background: 'var(--v2-surface2)', marginTop: 4 }} />}
              </div>
              {/* Card da etapa */}
              <div style={{ flex: 1, background: 'var(--v2-surface)', borderRadius: 14, padding: 18, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 4, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--v2-ink)' }}>{m.titulo}</span>
                  <span style={{ fontSize: 11, fontWeight: 700, color: STATUS_COR[m.status] || 'var(--v2-ink3)', background: `${STATUS_COR[m.status] || 'var(--v2-ink3)'}1a`, borderRadius: 999, padding: '3px 10px' }}>{STATUS_LABEL[m.status] || m.status}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', margin: '0 0 12px' }}>
                  <p style={{ margin: 0, fontSize: 12, color: 'var(--v2-ink3)' }}>{fmt(m.dataInicio)}{m.dataFim ? ` — prometido ${fmt(m.dataFim)}` : ''}</p>
                ...
                {m.descricao && <p style={{ margin: '0 0 12px', fontSize: 13, color: 'var(--v2-ink2)', lineHeight: 1.5 }}>{m.descricao}</p>}
