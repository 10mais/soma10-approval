...
export default function AprovacoesPagina() {
  ...
  return (
    <div>
      <h2 style={{ margin: '0 0 16px', fontSize: 18, color: 'var(--v2-ink)' }}>Aprovações</h2>
      ...
      {aguardando.length > 0 && (() => { const e = tempoEspera(maisAntiga); const urgente = e?.atrasado; return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', background: urgente ? 'var(--v2-hot-bg)' : 'var(--v2-amber-bg)', border: `1.5px solid ${urgente ? 'var(--v2-hot-bg)' : 'var(--v2-amber-bg)'}`, borderRadius: 12, padding: '14px 18px', marginBottom: 16 }}>
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            <p style={{ margin: '2px 0 0', fontSize: 12, color: urgente ? 'var(--v2-hot)' : 'var(--v2-amber)', opacity: 0.85 }}>Aprovar rápido mantém o ritmo das suas entregas.</p>
          </div>
            ...
            return (
              <div key={p.id} style={{ position: 'relative', background: 'var(--v2-surface)', borderRadius: 14, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', padding: '16px 18px' }}>
                ...
                <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  {capa && (
                    <div onClick={() => abrirViewer(p, 0)} title="Clique para ampliar" style={{ position: 'relative', width: 96, height: 96, borderRadius: 10, overflow: 'hidden', background: 'var(--v2-surface2)', flexShrink: 0, cursor: 'zoom-in' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {ehCopy && (p.headline || p.subheadline || p.textoImagem || p.cta) && <p style={{ margin: '0 0 2px', fontSize: 10.5, fontWeight: 800, color: 'var(--v2-ink3)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Legenda</p>}
                      ...
                      <div style={{ marginBottom: 8 }}>
                        <div style={{ display: 'flex', gap: 6, overflowX: 'auto' }}>
                          {p.imagens.map((m: string, i: number) => (
                            <div key={i} onClick={() => abrirViewer(p, i)} title="Clique para ampliar" style={{ width: 68, height: 68, borderRadius: 8, overflow: 'hidden', background: 'var(--v2-surface2)', flexShrink: 0, cursor: 'zoom-in' }}>
                    ...
                    {permAprovar ? (
                      ...
                      {p.status === 'corrigir' ? (
                        <div style={{ marginTop: 4 }}>
                          <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 10, padding: '10px 12px', marginBottom: 8 }}>
                            <p style={{ margin: 0, fontSize: 12.5, fontWeight: 800, color: 'var(--v2-amber)' }}>Em ajuste</p>
                          ...
                          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                            <button onClick={() => setAjuste({ id: p.id, legenda: p.legenda || '', obs: (ehCopy ? p.ajusteCopy : p.ajusteCriativo) || '', data: toLocalInput(p.dataAgendada) })} disabled={enviando === p.id}
                              style={{ padding: '8px 16px', background: 'var(--v2-amber-on)', color: '#17150E', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>Editar ajuste</button>
                            <button onClick={() => agir(p.id, ehCopy ? 'aprovar_copy' : 'aprovar_criativo')} disabled={enviando === p.id}
                              style={{ padding: '8px 16px', background: 'var(--v2-surface)', color: 'var(--v2-ok)', border: '1px solid var(--v2-ok-bg)', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>Aprovar assim mesmo</button>
                          </div>
                        </div>
                      ) : (
                        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                          <button onClick={() => agir(p.id, ehCopy ? 'aprovar_copy' : 'aprovar_criativo')} disabled={enviando === p.id}
                            style={{ padding: '8px 20px', background: 'var(--v2-ok)', color: 'var(--v2-surface)', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>Aprovar</button>
                          <button onClick={() => setAjuste({ id: p.id, legenda: p.legenda || '', obs: '', data: toLocalInput(p.dataAgendada) })} disabled={enviando === p.id}
                            style={{ padding: '8px 16px', background: 'var(--v2-amber-on)', color: '#17150E', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>Solicitar ajustes</button>
                          <button onClick={() => { setRejeitar({ id: p.id, ehCopy }); setMotivoRejeicao('') }} disabled={enviando === p.id}
                            style={{ padding: '8px 16px', background: 'var(--v2-surface)', color: 'var(--v2-hot)', border: '1px solid var(--v2-hot-bg)', borderRadius: 8, fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>Rejeitar</button>
                        </div>
                      )}
                      ...
                      {ajuste?.id === p.id && (() => {
                        ...
                        return (
                          <div style={{ marginTop: 10, background: '#fffdf7', border: '1px solid var(--v2-amber-bg)', borderRadius: 10, padding: 12 }}>
                            <p style={{ margin: '0 0 2px', fontSize: 13.5, fontWeight: 800, color: 'var(--v2-ink)' }}>Solicitar ajustes</p>
                            <p style={{ margin: '0 0 4px', fontSize: 11.5, color: 'var(--v2-ink3)', lineHeight: 1.5 }}>Peça tudo de uma vez. Nada é enviado até você clicar em <strong>Enviar solicitação</strong>.</p>
                            ...
                            {!ehCopy ? (<>
                              ...
                              <textarea lang="pt-BR" value={ajuste.obs} onChange={e => setAjuste(a => a && { ...a, obs: e.target.value })} rows={3} placeholder="Ex.: trocar a cor do título, aumentar a logo, mudar a foto..." style={campo} />
                              ...
                              <textarea lang="pt-BR" value={ajuste.obs} onChange={e => setAjuste(a => a && { ...a, obs: e.target.value })} rows={3} placeholder="O que ajustar no texto..." style={campo} />
                            </>)}
                            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 10, flexWrap: 'wrap' }}>
                              <button onClick={() => setAjuste(null)} style={{ padding: '8px 14px', background: 'var(--v2-surface2)', color: 'var(--v2-ink2)', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>Voltar</button>
                              ...
                              }} style={{ padding: '8px 18px', background: 'var(--v2-amber-on)', color: '#17150E', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{enviando === p.id ? '...' : 'Enviar solicitação'}</button>
                            </div>
                          </div>
                        )
                      })()}
                      ...
                      <p style={{ margin: 0, textAlign: 'right', fontSize: 12, color: 'var(--v2-ink3)', fontStyle: 'italic' }}>Somente visualização — a aprovação é feita pela equipe.</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      {rejeitar && (
        <div onClick={fecharFora(() => setRejeitar(null))} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--v2-surface)', borderRadius: 16, maxWidth: 440, width: '100%', padding: 22 }}>
            ...
            <p style={{ margin: '0 0 14px', fontSize: 12, color: 'var(--v2-ink3)' }}>Informe o motivo. O material voltará para a equipe.</p>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button disabled={!motivoRejeicao.trim()} onClick={async () => { await agir(rejeitar.id, rejeitar.ehCopy ? 'ajuste_copy' : 'ajuste_criativo', `REJEITADO: ${motivoRejeicao}`); setRejeitar(null) }}
                style={{ padding: '9px 20px', background: motivoRejeicao.trim() ? 'var(--v2-hot)' : 'var(--v2-surface2)', color: motivoRejeicao.trim() ? 'var(--v2-surface)' : 'var(--v2-ink3)', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: motivoRejeicao.trim() ? 'pointer' : 'not-allowed' }}>Confirmar</button>
            </div>
          </div>
        </div>
      )}
