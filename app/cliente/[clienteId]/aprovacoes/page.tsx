      ...
      {pendentes.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {pendentes.map(p => {
            ...
            return (
              <div key={p.id} style={{ position: 'relative', background: 'var(--v2-surface)', borderRadius: 14, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', padding: '16px 18px' }}>
                {podeExcluir && (
                ...
                <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  {capa && (
                    <div onClick={() => abrirViewer(p, 0)} title={tr('portal.clique-ampliar')} style={{ position: 'relative', width: 96, height: 96, borderRadius: 10, overflow: 'hidden', background: 'var(--v2-surface2)', flexShrink: 0, cursor: 'zoom-in' }}>
                      ...
                      <div style={{ position: 'absolute', right: 4, bottom: 4, width: 22, height: 22, borderRadius: 6, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--v2-surface)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.3-4.3M11 8v6M8 11h6"/></svg>
                      </div>
                    </div>
                  )}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      ...
                      <span style={{ background: p.status === 'corrigir' ? '#fff7ed' : ehCopy ? 'var(--v2-info-bg)' : 'var(--v2-amber-bg)', border: p.status === 'corrigir' ? '1px solid #fed7aa' : 'none', borderRadius: 999, padding: '3px 10px', fontSize: 11, fontWeight: 700, color: p.status === 'corrigir' ? 'var(--v2-amber)' : ehCopy ? 'var(--v2-info)' : 'var(--v2-amber)' }}>
                        {p.status === 'corrigir' ? tr('aprov.ajuste') : ehCopy ? tr('dash.aprovar-copy') : tr('dash.aprovar-criativo')}
                      </span>
                      {(() => { const e = tempoEspera(p.aguardandoDesde); return e ? (
                        <span style={{ background: e.atrasado ? 'var(--v2-hot-bg)' : 'var(--v2-surface2)', borderRadius: 999, padding: '3px 10px', fontSize: 11, fontWeight: 700, color: e.atrasado ? 'var(--v2-hot)' : 'var(--v2-ink3)' }}>
                          aguardando {e.texto}
                        </span>
                      ) : null })()}
                    </div>
                    ...
                    {ehCopy && (p.headline || p.subheadline || p.textoImagem || p.cta || p.medidas || p.localAplicacao || (p.laminas || []).length > 0) && (
                      <div style={{ background: 'var(--v2-surface1)', border: '1px solid var(--v2-rule2)', borderRadius: 10, padding: '10px 12px', marginBottom: 8 }}>
                        <p style={{ margin: '0 0 6px', fontSize: 10.5, fontWeight: 800, color: 'var(--v2-ink3)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{p.formato === 'grafico' ? tr('portal.material-grafico') : tr('aprov.texto-arte')}</p>
                        ...
                        {p.cta && <p style={{ margin: '2px 0 0', fontSize: 12.5, fontWeight: 800, color: 'var(--v2-amber)' }}>{p.cta}</p>}
                      </div>
                    )}
                    {ehCopy && (p.headline || p.subheadline || p.textoImagem || p.cta) && <p style={{ margin: '0 0 2px', fontSize: 10.5, fontWeight: 800, color: 'var(--v2-ink3)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{tr('aprov.legenda')}</p>}
                    <p style={{ margin: '0 0 6px', fontSize: 13, color: 'var(--v2-ink)', whiteSpace: 'pre-wrap', maxHeight: 120, overflow: 'auto', lineHeight: 1.5 }}>{p.legenda || '(sem texto)'}</p>
                    {(p.imagens || []).length > 0 && !ehCopy && (
                      <div style={{ marginBottom: 8 }}>
                        <div style={{ display: 'flex', gap: 6, overflowX: 'auto' }}>
                          {p.imagens.map((m: string, i: number) => (
                            <div key={i} onClick={() => abrirViewer(p, i)} title={tr('portal.clique-ampliar')} style={{ width: 68, height: 68, borderRadius: 8, overflow: 'hidden', background: 'var(--v2-surface2)', flexShrink: 0, cursor: 'zoom-in' }}>
                        ...
                        <p style={{ margin: '5px 0 0', fontSize: 11, color: 'var(--v2-ink3)' }}>Toque em uma imagem para ampliar{p.imagens.length > 1 ? ` — ${p.imagens.length} itens no carrossel` : ''}.</p>
                      </div>
                    )}
                    {permAprovar ? (
                      <>
                      {p.status === 'corrigir' ? (
                        <div style={{ marginTop: 4 }}>
                          <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 10, padding: '10px 12px', marginBottom: 8 }}>
                            <p style={{ margin: 0, fontSize: 12.5, fontWeight: 800, color: 'var(--v2-amber)' }}>{tr('aprov.ajuste')}</p>
                            <p style={{ margin: '2px 0 0', fontSize: 12, color: '#9a6b2e', lineHeight: 1.5 }}>{tr('portal.seu-pedido-foi-enviado-agencia')}</p>
                            {(ehCopy ? p.ajusteCopy : p.ajusteCriativo) && <p style={{ margin: '6px 0 0', fontSize: 12.5, color: '#7c4a12' }}><strong>{tr('portal.voce-pediu')}</strong> {ehCopy ? p.ajusteCopy : p.ajusteCriativo}</p>}
                          </div>
                          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                            <button onClick={() => setAjuste({ id: p.id, legenda: p.legenda || '', obs: (ehCopy ? p.ajusteCopy : p.ajusteCriativo) || '', data: toLocalInput(p.dataAgendada) })} disabled={enviando === p.id}
                              style={{ padding: '8px 16px', background: 'var(--v2-amber-on)', color: '#17150E', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{tr('aprov.editar-ajuste')}</button>
                            <button onClick={() => agir(p.id, ehCopy ? 'aprovar_copy' : 'aprovar_criativo')} disabled={enviando === p.id}
                              style={{ padding: '8px 16px', background: 'var(--v2-surface)', color: 'var(--v2-ok)', border: '1px solid var(--v2-ok-bg)', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{tr('aprov.aprovar-assim-mesmo')}</button>
                          </div>
                        </div>
                      ) : (
                        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                          <button onClick={() => agir(p.id, ehCopy ? 'aprovar_copy' : 'aprovar_criativo')} disabled={enviando === p.id}
                            style={{ padding: '8px 20px', background: 'var(--v2-ok)', color: 'var(--v2-surface)', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{tr('aprov.aprovar')}</button>
                          <button onClick={() => setAjuste({ id: p.id, legenda: p.legenda || '', obs: '', data: toLocalInput(p.dataAgendada) })} disabled={enviando === p.id}
                            style={{ padding: '8px 16px', background: 'var(--v2-amber-on)', color: '#17150E', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{tr('aprov.solicitar-ajustes')}</button>
                          <button onClick={() => { setRejeitar({ id: p.id, ehCopy }); setMotivoRejeicao('') }} disabled={enviando === p.id}
                            style={{ padding: '8px 16px', background: 'var(--v2-surface)', color: 'var(--v2-hot)', border: '1px solid var(--v2-hot-bg)', borderRadius: 8, fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>{tr('aprov.rejeitar')}</button>
                        </div>
                      )}
                      ...
                      {ajuste?.id === p.id && (() => {
                        ...
                        const campo: React.CSSProperties = { width: '100%', boxSizing: 'border-box', padding: '10px 12px', borderRadius: 8, border: '1.5px solid var(--v2-rule)', fontSize: 13, fontFamily: 'inherit', resize: 'vertical', lineHeight: 1.5 }
                        const rot: React.CSSProperties = { display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--v2-ink2)', margin: '10px 0 5px' }
                        return (
                          <div style={{ marginTop: 10, background: '#fffdf7', border: '1px solid var(--v2-amber-bg)', borderRadius: 10, padding: 12 }}>
                            <p style={{ margin: '0 0 2px', fontSize: 13.5, fontWeight: 800, color: 'var(--v2-ink)' }}>{tr('aprov.solicitar-ajustes')}</p>
                            <p style={{ margin: '0 0 4px', fontSize: 11.5, color: 'var(--v2-ink3)', lineHeight: 1.5 }}>{tr('portal.peca-tudo', { botao: tr('aprov.enviar-solicitacao') })}</p>
                            <label style={rot}>{tr('aprov.legenda')}</label>
                            <textarea lang="pt-BR" value={ajuste.legenda} onChange={e => setAjuste(a => a && { ...a, legenda: e.target.value })} rows={4} style={campo} />
                            {!ehCopy ? (<>
                              <label style={rot}>{tr('portal.que-ajustar-layout')}</label>
                              <textarea lang="pt-BR" value={ajuste.obs} onChange={e => setAjuste(a => a && { ...a, obs: e.target.value })} rows={3} placeholder={tr('portal.ex-trocar-cor-titulo-aumentar')} style={campo} />
                              <label style={rot}>{tr('aprov.data-horario-publicacao')}</label>
                              <input type="datetime-local" value={ajuste.data} onChange={e => setAjuste(a => a && { ...a, data: e.target.value })} style={campo} />
                            </>) : (<>
                              <label style={rot}>{tr('portal.observacao-opcional')}</label>
                              <textarea lang="pt-BR" value={ajuste.obs} onChange={e => setAjuste(a => a && { ...a, obs: e.target.value })} rows={3} placeholder={tr('portal.que-ajustar-texto')} style={campo} />
                            </>)}
                            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 10, flexWrap: 'wrap' }}>
                              <button onClick={() => setAjuste(null)} style={{ padding: '8px 14px', background: 'var(--v2-surface2)', color: 'var(--v2-ink2)', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>{tr('aprov.voltar')}</button>
                              <button disabled={enviando === p.id} onClick={() => {
                                if (!ajuste.obs.trim() && !legendaMudou && !dataMudou) { toast(tr('aprov.faca-um-ajuste'), 'erro'); return }
                                const obs = ajuste.obs.trim(); const nl = legendaMudou ? ajuste.legenda : undefined; const nd = dataMudou ? ajuste.data : undefined
                                setAjuste(null)
                                agir(p.id, ehCopy ? 'ajuste_copy' : 'ajuste_criativo', obs || tr('portal.ajuste-solicitado'), nl, nd)
                              }} style={{ padding: '8px 18px', background: 'var(--v2-amber-on)', color: '#17150E', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{enviando === p.id ? '...' : tr('aprov.enviar-solicitacao')}</button>
                            </div>
                          </div>
                        )
                      })()}
                      ...
                      <p style={{ margin: 0, textAlign: 'right', fontSize: 12, color: 'var(--v2-ink3)', fontStyle: 'italic' }}>{tr('portal.somente-visualizacao-aprovacao')}</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      {rejeitar && (
        <div onClick={fecharFora(() => setRejeitar(null))} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--v2-surface)', borderRadius: 16, maxWidth: 440, width: '100%', padding: 22 }}>
            ...
            <p style={{ margin: '0 0 14px', fontSize: 12, color: 'var(--v2-ink3)' }}>{tr('portal.informe-motivo-material-voltar')}</p>
            <textarea lang="pt-BR" value={motivoRejeicao} onChange={e => setMotivoRejeicao(e.target.value)} placeholder={tr('aprov.motivo-rejeicao')}
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button disabled={!motivoRejeicao.trim()} onClick={async () => { await agir(rejeitar.id, rejeitar.ehCopy ? 'ajuste_copy' : 'ajuste_criativo', `REJEITADO: ${motivoRejeicao}`); setRejeitar(null) }}
                style={{ padding: '9px 20px', background: motivoRejeicao.trim() ? 'var(--v2-hot)' : 'var(--v2-surface2)', color: motivoRejeicao.trim() ? 'var(--v2-surface)' : 'var(--v2-ink3)', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: motivoRejeicao.trim() ? 'pointer' : 'not-allowed' }}>{tr('portal.confirmar')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {viewer && (() => {
        ...
        return (
          <div onClick={fecharFora(() => setViewer(null), { perguntar: false })} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: 20 }}>
            <button onClick={() => setViewer(null)} title={tr('crm.fechar-esc')} style={{ position: 'absolute', top: 16, right: 16, width: 40, height: 40, borderRadius: '50%', background: 'rgba(255,255,255,0.12)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--v2-surface)" strokeWidth="2.4" strokeLinecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg>
            </button>
            {varios && (
              <button onClick={e => { e.stopPropagation(); ir(-1) }} title={tr('dash.anterior')} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 46, height: 46, borderRadius: '50%', background: 'rgba(255,255,255,0.12)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--v2-surface)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
              </button>
            )}
            <div onClick={e => e.stopPropagation()} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
              {ehVideoUrl(cur)
                ? <video src={cur} controls autoPlay style={{ maxWidth: '92vw', maxHeight: '82vh', borderRadius: 10, background: 'var(--v2-ink)' }} />
                : <img src={cur} alt="" style={{ maxWidth: '92vw', maxHeight: '82vh', objectFit: 'contain', borderRadius: 10 }} />}
              {varios && <span style={{ color: 'var(--v2-surface)', fontSize: 13, fontWeight: 600, opacity: 0.85 }}>{viewer.idx + 1} / {viewer.midias.length}</span>}
            </div>
            {varios && (
              <button onClick={e => { e.stopPropagation(); ir(1) }} title={tr('portal.proximo')} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', width: 46, height: 46, borderRadius: '50%', background: 'rgba(255,255,255,0.12)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--v2-surface)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6"/></svg>
              </button>
            )}
          </div>
        )
