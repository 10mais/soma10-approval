        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 14, marginBottom: 20 }}>
            {[
              ...
              <div key={k.l} style={{ background: '#fff', borderRadius: 14, padding: '16px 18px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                <p style={{ margin: '0 0 6px', fontSize: 11, fontWeight: 700, color: '#aaa', textTransform: 'uppercase' }}>{k.l}</p>
                <p style={{ margin: 0, fontSize: 24, fontWeight: 800, color: '#111' }}>{(k.v ?? 0).toLocaleString('pt-BR')}</p>
              </div>
            ))}
          </div>
          {(data.posts || []).length > 0 && (
            <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
              <p style={{ margin: 0, padding: '16px 20px', fontSize: 13, fontWeight: 700, color: '#111', borderBottom: '1px solid #f0f0f0' }}>Posts por relevancia ({data.posts.length})</p>
              {data.posts.map((p: any) => (
                <div key={p.id} style={{ padding: '12px 20px', borderBottom: '1px solid #f8f8f8', display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 12, color: '#333', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.legenda?.slice(0, 80) || '--'}</p>
                    <p style={{ margin: '2px 0 0', fontSize: 11, color: '#aaa' }}>{p.curtidas} curtidas · {p.comentarios} comentarios · {p.alcance} alcance</p>
                  </div>
                  {p.link && <a href={p.link} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: '#1d4ed8' }}>Ver</a>}
                </div>
              ))}
            </div>
          )}
