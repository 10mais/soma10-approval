            ...
            {[{ titulo: 'Clientes', lista: clientes.filter(c => (c as any).tipo !== 'interno') }, { titulo: 'Projetos internos', lista: clientes.filter(c => (c as any).tipo === 'interno') }].map(g => g.lista.length === 0 ? null : (
              ...
              {g.lista.map(c => (
                <div key={c.id} style={{ background: '#fff', borderRadius: 14, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', overflow: 'hidden', height: 'fit-content' }}>
                  ...
                  {/* Barra de ações (aparece ao expandir) — separada do cabeçalho, agrupada por intenção */}
                  {clienteAberto === c.id && (() => {
                    ...
                    const rotulo: React.CSSProperties = { flexShrink: 0, width: 84, fontSize: 10.5, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.04em' }
                    const btn: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 7, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 9, padding: '7px 13px', fontSize: 12.5, fontWeight: 600, color: '#374151', cursor: 'pointer', lineHeight: 1, whiteSpace: 'nowrap' }
                    const chip: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 6, borderRadius: 999, padding: '5px 11px', fontSize: 12, fontWeight: 600, lineHeight: 1 }
                    const linha: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }
                    ...
                    return (
                      <div onClick={e => e.stopPropagation()} style={{ borderTop: '1px solid #f3f4f6', padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                        {/* Compartilhar com o cliente */}
                        {c.tipo !== 'interno' && (
                          <div style={linha}>
                            <span style={rotulo}>Compartilhar</span>
                            <button onClick={() => abrirResumo(c.id)} title="Gerar o resumo da semana para enviar ao cliente" style={btn}>
                            ...
                            <button onClick={() => statusPublico(c.id)} title="Abrir/copiar o link público de status (sem login) para enviar ao cliente" style={btn}>
                            ...
                            <button onClick={() => linkAprovacao(c.id)} title="Copiar o link ÚNICO de aprovação (sem login) — mostra todos os materiais aguardando aprovação deste cliente"
                              style={{ ...btn, background: '#111', border: '1px solid #111', color: '#fff' }}>
                        ...
                        <div style={linha}>
                          ...
                          {temIG && <span style={{ ...chip, background: '#fdecf3', color: '#c2185b' }}>{igIcon}Instagram{c.instagramUsername ? ' · @' + c.instagramUsername : ''}</span>}
                          {temFB && <span style={{ ...chip, background: '#e7f0fd', color: '#1877f2' }}>{fbIcon}Facebook</span>}
                          {role === 'admin' ? (
                            <>
                              {!temIG && <a href={`/api/instagram/oauth?cliente=${c.id}`} style={{ ...btn, textDecoration: 'none', color: '#c2185b' }}>{igIcon} Conectar Instagram</a>}
                              {!temFB && <a href={`/api/meta/oauth?cliente=${c.id}`} style={{ ...btn, textDecoration: 'none', color: '#1877f2' }}>{fbIcon} Conectar Facebook</a>}
                              {(temFB || temIG) && <button onClick={async () => { if (await confirmar(`Desconectar as redes sociais de ${c.nome}? O perfil perdera o acesso para publicacao ate ser reconectado.`, { titulo: 'Desconectar redes', okLabel: 'Desconectar', perigo: true })) desconectarInstagram(c.id) }} style={{ ...btn, color: '#9ca3af', padding: '7px 11px' }}>Desconectar</button>}
                            </>
                          ) : (!temFB && !temIG) ? (
                            <span style={{ ...chip, background: '#fff7ed', color: '#b45309' }}>Não conectado</span>
                          ) : null}
