...
export async function POST(req: NextRequest) {
  ...
  const { nome, email, senha, role, cargo, clienteId, custoHora, salarioFixo, salarioVariavel } = await req.json()
  ...
  const usuario: Usuario = { id: uuid(), nome, email, senha: hash, role, cargo: cargo || '', custoHora: Number(custoHora) || 0, salarioFixo: Number(salarioFixo) || 0, salarioVariavel: Number(salarioVariavel) || 0, ...(role === 'cliente' && clienteId ? { clienteId } : {}), criadoEm: new Date().toISOString() }
...
export async function PUT(req: NextRequest) {
  ...
  const { email, nome, role, novaSenha, cargo, foto, clienteId, custoHora, salarioFixo, salarioVariavel } = await req.json()
  ...
  if (salarioVariavel !== undefined) usuario.salarioVariavel = Number(salarioVariavel) || 0
