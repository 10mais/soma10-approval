...
export async function POST(req: NextRequest) {
  ...
  const { nome, email, senha, role, cargo, funcaoVendas, areaSaude, corAgenda, recebeAgenda, permissoes, permissoesGranular, clienteId, custoHora, salarioFixo, valorPorProjeto, qtdProjetos } = await req.json()
  ...
  const usuario: Usuario = { id: uuid(), nome, email, senha: hash, role, cargo: cargo || '', custoHora: Number(custoHora) || 0, salarioFixo: Number(salarioFixo) || 0, valorPorProjeto: Number(valorPorProjeto) || 0, qtdProjetos: Number(qtdProjetos) || 0, salarioVariavel: (Number(valorPorProjeto) || 0) * (Number(qtdProjetos) || 0), ...(role === 'cliente' && clienteId ? { clienteId } : {}), ...(role === 'vendas' && funcaoVendas ? { funcaoVendas } : {}), ...(areaSaude ? { areaSaude } : {}), ...(corAgenda ? { corAgenda } : {}), ...(typeof recebeAgenda === 'boolean' ? { recebeAgenda } : {}), ...((role === 'gerente' || role === 'usuario') && permissoes ? { permissoes } : {}), ...((role === 'gerente' || role === 'usuario') && permissoesGranular ? { permissoesGranular } : {}), criadoEm: new Date().toISOString() }
...
export async function PUT(req: NextRequest) {
  ...
  const { email, nome, role, novaSenha, cargo, funcaoVendas, areaSaude, corAgenda, recebeAgenda, permissoes, permissoesGranular, foto, clienteId, custoHora, salarioFixo, valorPorProjeto, qtdProjetos, resetar2FA } = await req.json()
