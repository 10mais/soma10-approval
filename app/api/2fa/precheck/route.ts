...
// para a tela de login mostrar (ou não) o campo de código. Rate-limited.
export async function POST(req: NextRequest) {
  ...
  if (!senhaOk) return NextResponse.json({ ok: false })
  return NextResponse.json({ ok: true, needs2FA: !!u.twoFactorEnabled })
}
